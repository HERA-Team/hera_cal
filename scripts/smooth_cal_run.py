...
def run():
    if a.run_if_first is None or sorted(a.calfits_list)[0] == a.run_if_first:
        cs = CalibrationSmoother(
            a.calfits_list,
            flag_file_list=a.flag_file_list,
            flag_filetype=a.flag_filetype,
            antflag_thresh=a.antflag_thresh,
            time_blacklists=a.time_blacklists,
            lst_blacklists=a.lst_blacklists,
            freq_blacklists=a.freq_blacklists,
            blacklist_wgt=a.blacklist_wgt,
            chan_blacklists=a.chan_blacklists,
            pick_refant=a.pick_refant,
            freq_threshold=a.freq_threshold,
            time_threshold=a.time_threshold,
            ant_threshold=a.ant_threshold,
            verbose=a.verbose,
        )
        if a.axis == "both":
            cs.time_freq_2D_filter(
                freq_scale=a.freq_scale,
