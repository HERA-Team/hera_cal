...
if ap.mode == "clean":
    filter_kwargs = {
        "window": ap.window,
        "maxiter": ap.maxiter,
        "edgecut_hi": ap.edgecut_hi,
        "edgecut_low": ap.edgecut_low,
        "gain": ap.gain,
    }
    if ap.window == "tukey":
...
if ap.cornerturnfile is not None:
    baseline_list = io.baselines_from_filelist_position(
        filename=ap.cornerturnfile, filelist=ap.datafilelist
    )
else:
    baseline_list = None
...
run_with_profiling(
    ...
    ap,
    ap.datafilelist,
    calfile_list=ap.calfilelist,
    avg_red_bllens=True,
    baseline_list=baseline_list,
    spw_range=ap.spw_range,
    cache_dir=ap.cache_dir,
    filled_outfilename=ap.filled_outfilename,
    clobber=ap.clobber,
    write_cache=ap.write_cache,
    CLEAN_outfilename=ap.CLEAN_outfilename,
    read_cache=ap.read_cache,
    mode=ap.mode,
    res_outfilename=ap.res_outfilename,
    factorize_flags=ap.factorize_flags,
    time_thresh=ap.time_thresh,
    wgt_by_nsample=ap.wgt_by_nsample,
    lst_blacklists=ap.lst_blacklists,
    blacklist_wgt=ap.blacklist_wgt,
    add_to_history=" ".join(sys.argv),
    verbose=ap.verbose,
    flag_yaml=ap.flag_yaml,
    Nbls_per_load=ap.Nbls_per_load,
    case=ap.case,
    external_flags=ap.external_flags,
    filter_spw_ranges=ap.filter_spw_ranges,
    overwrite_flags=ap.overwrite_flags,
    skip_autos=ap.skip_autos,
    skip_if_flag_within_edge_distance=ap.skip_if_flag_within_edge_distance,
    zeropad=ap.zeropad,
    tol=ap.tol,
    skip_wgt=ap.skip_wgt,
    max_frate_coeffs=ap.max_frate_coeffs,
    frate_width_multiplier=ap.frate_width_multiplier,
    frate_standoff=ap.frate_standoff,
    fr_freq_skip=ap.fr_freq_skip,
    min_frate_half_width=ap.min_frate_half_width,
    max_frate_half_width=ap.max_frate_half_width,
    beamfitsfile=ap.beamfitsfile,
    percentile_low=ap.percentile_low,
