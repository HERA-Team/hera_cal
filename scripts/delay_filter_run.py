...
if a.partial_load_Nbls is not None:  # partial loading
    delay_filter.partial_load_delay_filter_and_write(a.infilename, calfile=a.calfile, Nbls=a.partial_load_Nbls,
        res_outfilename=a.res_outfilename, CLEAN_outfilename=a.CLEAN_outfilename, filled_outfilename=a.filled_outfilename,
        clobber=a.clobber, add_to_history=' '.join(sys.argv), **filter_kwargs)
else:
    df = delay_filter.Delay_Filter()
    df.load_data(a.infile, filetype=a.filetype_in, input_cal=a.calfile)
