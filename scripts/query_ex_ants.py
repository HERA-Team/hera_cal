...
def query_ex_ants(JD, good_statuses):
    ...
    valid_statuses = (
        list(h.apriori.values())[0].valid_statuses()
        + list(h.apriori.values())[0].old_statuses()
    )
    ...
    for ant in h.apriori:
        if h.apriori[ant].status not in good_statuses:
            exants.append(
                int(ant[2:].split(":")[0])
            )  # Assumes the format HH0:A or HA330:A
...
a = argparse.ArgumentParser(
    description='Command line function for printing out space-separated "bad" antennas on a given JD.'
)
a.add_argument("JD", type=float, help="Julian data on which to query the database.")
a.add_argument(
    "good_statuses",
    type=str,
    help="Comma-separated list of acceptable anntenna statuses. Antennas "
