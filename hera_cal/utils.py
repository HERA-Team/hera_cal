...
def broadcast_flags(flags, spw_ranges=None, time_thresh=0.05, inplace=False):
    ...
    if isinstance(flags, np.ndarray):
        ...
        for (spw1, spw2) in spw_ranges:
            ...
            exceeds_thresh = np.sum(flags[~fully_flagged_ints, spw1:spw2], axis=0, dtype=np.float) / unflagged_Ntimes > time_thresh
    ...
    elif isinstance(flags, hera_cal.datacontainer.DataContainer):
