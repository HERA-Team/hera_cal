...
class DataContainer:
    ...
    def pols(self, bl=None):
        '''Return a set of polarizations (with a specific bl or more generally).'''
        if bl is None:
    ...
    def has_key(self, *args):
        ...
        if len(args) == 1:
            return (self._convert_case(args[0]) in self._data or
                    self._convert_case(self._switch_bl(args[0])) in self._data)
        else:
    ...
    def has_bl(self, bl):
        ...
        return bl in self._bls or self._switch_bl(bl) in self._bls
