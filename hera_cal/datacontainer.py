...
class DataContainer:
            ...
            if isinstance(key, str):  # asking for a pol
                ...
                try:
                    ...
                    try:
                        ...
                        raise KeyError(
                            f"Cannot find either {key} or {reverse_bl(key)} in this DataContainer."
                        )
        ...
        if not skip_bda_check:
            if dc.times_by_bl is not None:
                for tbbl in dc.times_by_bl.values():
                    assert np.all(
                        tbbl == np.asarray(dc.times)
                    ), "select_or_expand_times does not support baseline dependent averaging."
            if dc.lsts_by_bl is not None:
                for lbbl in dc.lsts_by_bl.values():
                    assert np.all(
                        lbbl == np.asarray(dc.lsts)
                    ), "select_or_expand_times does not support baseline dependent averaging."

        # update data
        nt_inds = np.searchsorted(np.array(dc.times), np.array(new_times))
        for bl in dc:
            assert dc[bl].shape[0] == len(
                dc.times
            ), "select_or_expand_times assume that time is the 0th data dimension."
            dc[bl] = dc[bl][nt_inds]
    ...
    def select_freqs(
        ...
        if obj.freqs is None:
            warnings.warn(
                "It is impossible to automatically detect which axis is frequency. Trying last axis."
            )
            axis = -1
        else:
    ...
    def __init__(
        ...
        if reds is None:
            ...
            if antpos is not None:
                ...
                raise ValueError(
                    "Must provide reds, antpos, or have antpos available at data.antpos"
                )
