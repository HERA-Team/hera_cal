...
def construct_version_info():
    ...
    try:
        git_origin = subprocess.check_output(['git', '-C', hera_cal_dir, 'config',
                                              '--get', 'remote.origin.url'],
                                             stderr=subprocess.STDOUT).strip()
        git_hash = subprocess.check_output(['git', '-C', hera_cal_dir, 'rev-parse', 'HEAD'],
                                           stderr=subprocess.STDOUT).strip()
        git_description = subprocess.check_output(['git', '-C', hera_cal_dir,
                                                   'describe', '--dirty', '--tag', '--always']).strip()
        git_branch = subprocess.check_output(['git', '-C', hera_cal_dir, 'rev-parse',
                                              '--abbrev-ref', 'HEAD'],
                                             stderr=subprocess.STDOUT).strip()
        git_version = subprocess.check_output(['git', '-C', hera_cal_dir, 'describe',
                                               '--tags', '--abbrev=0']).strip()
    except BaseException:  # pragma: no cover  - can't figure out how to test exception.
        try:
...
# String to add to history of any files written with this version of hera_cal
