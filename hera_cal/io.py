...
def _parse_input_files(inputs, name="input_data"):
    if isinstance(inputs, (str, Path)):
        ...
        if np.all(
            [isinstance(i, (str, Path)) for i in inputs]
        ):  # List of visibility data paths
            filepaths = list(inputs)
        else:
            raise TypeError(
                f"If {name} is a list, it must be a list of strings or Paths."
            )
    else:
...
class HERACal(UVCal):
    ...
    def _extract_metadata(self):
        ...
        self.pols = [
            jnum2str(j, x_orientation=self.x_orientation) for j in self.jones_array
        ]
    ...
    def build_calcontainers(self):
        ...
        for ant, pol in self.ants:
            i, ip = (
                self._antnum_indices[ant],
                self._jnum_indices[jstr2num(pol, x_orientation=self.x_orientation)],
            )
            ...
            if antenna_nums is not None:
                for ant in antenna_nums:
                    if ant not in my_ants:
                        warnings.warn(
                            f"Warning, antenna {ant} not present in calibration solution. Skipping!"
                        )
    ...
    def update(
            ...
            if to_update is not None:
                for ant, pol in to_update.keys():
                    i, ip = (
                        self._antnum_indices[ant],
                        self._jnum_indices[
                            jstr2num(pol, x_orientation=self.x_orientation)
                        ],
                    )
        ...
        if total_qual is not None:
            if self.total_quality_array is None:
                self.total_quality_array = np.zeros(
                    self.gain_array.shape[1:], dtype=float
                )
    ...
    def write(self, filename, spoof_missing_channels=False, **write_kwargs):
        ...
        if spoof_missing_channels:
            ...
            freqs_filled, _, _, inserted = place_data_on_uniform_grid(
                self.freqs, np.ones_like(self.freqs), np.ones_like(self.freqs)
            )
            writer.freq_array = freqs_filled.flatten()
            writer.Nfreqs = len(freqs_filled)
            writer.channel_width = np.median(writer.channel_width) * np.ones_like(
                writer.freq_array
            )
            if (
                hasattr(writer, "flex_spw_id_array")
                and writer.flex_spw_id_array is not None
            ):
                writer.flex_spw_id_array = np.full(
                    writer.Nfreqs, writer.spw_array[0], dtype=int
                )
            # insert original flags and gains into appropriate channels.
            new_gains = np.ones(
                (writer.Nants_data, writer.Nfreqs, writer.Ntimes, writer.Njones),
                dtype=complex,
            )
            ...
            new_total_quality = np.ones(
                (writer.Nfreqs, writer.Ntimes, writer.Njones), dtype=float
            )
            new_total_quality[~inserted, :, :] = writer.total_quality_array

            if writer.total_quality_array is not None:
                new_total_quality = np.zeros(
                    (writer.Nfreqs, writer.Ntimes, writer.Njones), dtype=float
                )
                new_total_quality[~inserted, :, :] = writer.total_quality_array
...
def read_hera_calfits(
    ...
    for cnt, filename in enumerate(filenames):
        with fits.open(filename) as fname:
            ...
            if _jhash not in inds:
                ...
                _pols = [
                    uvutils.parse_jpolstr(
                        uvutils.JONES_NUM2STR_DICT[num], x_orientation=x_orient
                    )
            ...
            if check:
                ...
                assert (
                    cal_type == "gain"
                )  # delay-style calibration currently unsupported
    ...
    if ants is None:
        ...
        ants = set(
            (ant,) if np.issubdtype(type(ant), np.integer) else ant for ant in ants
        )
    ...
    if read_quality:
        rv["quality"] = {
            ant: nan_empty((tot_times, nfreqs), np.float32) for ant in ants
        }
    if read_tot_quality:
        rv["total_quality"] = {
            p: nan_empty((tot_times, nfreqs), np.float32) for p in info["pols"]
        }
                ...
                if read_gains:
                    ...
                    for a, p in rv["gains"].keys():
                        if a not in antind or p not in polind:
                            continue
                        rv["gains"][a, p][t : t + ntimes].real = (
                            fname[0].data[antind[a], 0, :, :, polind[p], 0].T
                        )
                        rv["gains"][a, p][t : t + ntimes].imag = (
                            fname[0].data[antind[a], 0, :, :, polind[p], 1].T
                        )
                if read_flags:
                    for a, p in rv["flags"].keys():
                        if a not in antind or p not in polind:
                            continue
                        rv["flags"][a, p][t : t + ntimes] = (
                            fname[0].data[antind[a], 0, :, :, polind[p], 2].T
                        )
                if read_quality:
                    for a, p in rv["quality"].keys():
                        if a not in antind or p not in polind:
                            continue
                        rv["quality"][a, p][t : t + ntimes] = (
                            fname[0].data[antind[a], 0, :, :, polind[p], 3].T
                        )
                if read_tot_quality:
                    ...
                    for p in rv["total_quality"].keys():
                        if p not in polind:
                            continue
                        rv["total_quality"][p][t : t + ntimes] = tq_hdu.data[
                            0, :, :, polind[p]
                        ].T
        t += ntimes
    rv["info"] = info
    ...
    if getattr(uvo, "blts_are_rectangular", False):
        ...
        for ant1, ant2 in uvo.get_antpairs():
            ...
            if len(indices) == 1:  # only one blt matches
                ...
                len(set(np.ediff1d(indices))) == 1
            ):  # checks if the consecutive differences are all the same
                ...
                blt_slices[(ant1, ant2)] = slice(
                    indices[0], indices[-1] + 1, indices[1] - indices[0]
                )
    return blt_slices
...
class HERAData(UVData):
    ...
    * Generators that enable iterating over baseline, frequency, or time in chunks (see
    ...
    def __init__(
        ...
        **read_kwargs,
        ...
        if self.filetype in ["uvh5", "uvfits"]:
            ...
            raise NotImplementedError(
                "Filetype " + self.filetype + " has not been implemented."
            )
    ...
    def _attach_metadata(self, **read_kwargs):
        ...
        """
        if (
            hasattr(self, "filepaths")
            and self.filepaths is not None
            and len(self.filepaths) > 1
        ):  # save HERAData_metas in dicts
            for meta in self.HERAData_metas:
                setattr(self, meta, {})
    ...
    def get_metadata_dict(self):
        ...
        pols = [
            polnum2str(polnum, x_orientation=self.x_orientation)
            for polnum in self.polarization_array
        ]
        antpairs = self.get_antpairs()
        bls = [antpair + (pol,) for antpair in antpairs for pol in pols]

        times_by_bl = {
            antpair: np.array(self.time_array[self._blt_slices[antpair]])
            for antpair in antpairs
        }
        times_by_bl.update(
            {
                (ant1, ant0): times_here
                for (ant0, ant1), times_here in times_by_bl.items()
            }
        )
        lsts_by_bl = {
            antpair: np.array(self.lst_array[self._blt_slices[antpair]])
            for antpair in antpairs
        }
        lsts_by_bl.update(
            {(ant1, ant0): lsts_here for (ant0, ant1), lsts_here in lsts_by_bl.items()}
        )
    ...
    def _get_slice(self, data_array, key):
        ...
        if isinstance(key, str):  # asking for a pol
            return {
                antpair: self._get_slice(data_array, antpair + (key,))
                for antpair in self.get_antpairs()
            }
    ...
    def _set_slice(self, data_array, key, value):
        ...
        if isinstance(key, str):  # providing pol with all antpairs
            ...
            try:
                ...
                pidx = self.get_polstr_index(conj_pol(key[2]))
                data_array[self._blt_slices[tuple(key[1::-1])], :, pidx] = np.conj(
                    value
                )
        else:
            raise KeyError("Unrecognized key type for slicing data.")
        ...
        if self.filepaths is not None:
            ...
            try:
                if self.filetype in ["uvh5", "uvfits"]:
                    ...
                    if not read_data:
                        raise NotImplementedError(
                            "reading only metadata is not implemented for "
                            + self.filetype
                        )
                    if self.filetype == "miriad":
                        ...
                        if any(
                            [
                                times is not None,
                                lsts is not None,
                                lst_range is not None,
                                frequencies is not None,
                                freq_chans is not None,
                            ]
                        ):
                            warnings.warn(
                                "miriad does not support partial loading for times/lsts (except time_range) and frequencies. "
                ...
                if self.upsample:
                    if (
                        hasattr(self, "shortest_integration")
                        and self.shortest_integration is not None
                    ):
                        self.upsample_in_time(max_int_time=self.shortest_integration)
                    else:
                        self.upsample_in_time(
                            max_int_time=np.min(self.integration_time)
                        )
                if self.downsample:
                    if (
                        hasattr(self, "longest_integration")
                        and self.longest_integration is not None
                    ):
                        self.downsample_in_time(min_int_time=self.longest_integration)
                    else:
                        self.downsample_in_time(
                            min_int_time=np.max(self.integration_time)
                        )
    ...
    def update(self, data=None, flags=None, nsamples=None, tSlice=None, fSlice=None):
        ...
        """
        if data is not None:
            self.set_data_array_with_datacontainer(
                data, self.data_array, tSlice=tSlice, fSlice=fSlice
            )
        if flags is not None:
            self.set_data_array_with_datacontainer(
                flags, self.flag_array, tSlice=tSlice, fSlice=fSlice
            )
        if nsamples is not None:
            self.set_data_array_with_datacontainer(
                nsamples, self.nsample_array, tSlice=tSlice, fSlice=fSlice
            )
    ...
    def set_data_array_with_datacontainer(
        ...
        if data_array.shape != self.data_array.shape:
            raise ValueError(
                f"data_array must have shape {self.data_array.shape} (same as self.data_array)"
            )
    ...
    def partial_write(
        ...
        **kwargs,
        ...
        if self.filetype != "uvh5":
            raise NotImplementedError(
                "Partial writing for filetype "
                + self.filetype
                + " has not been implemented."
            )
        if len(self.filepaths) > 1:
            raise NotImplementedError(
                "Partial writing for list-loaded HERAData objects has not been implemented."
            )

        # get writer or initialize new writer if necessary
        if output_path in self._writers:
            hd_writer = self._writers[
                output_path
            ]  # This hd_writer has metadata for the entire output file
        else:
            ...
            hd_writer.initialize_uvh5_file(
                output_path, clobber=clobber
            )  # Makes an empty file (called only once)
        ...
        if inplace:  # update this objects's arrays using DataContainers
            ...
            n = self.set_data_array_with_datacontainer(
                nsamples, self.nsample_array.copy()
            )
    ...
    def iterate_over_bls(
        self,
        Nbls=1,
        bls=None,
        chunk_by_redundant_group=False,
        reds=None,
        bl_error_tol=1.0,
        include_autos=True,
        frequencies=None,
        ...
        if bls is None:
            if self.filetype != "uvh5":
                raise NotImplementedError(
                    "Baseline iteration without explicitly setting bls for filetype "
                    + self.filetype
        ...
        if not chunk_by_redundant_group:
            ...
            if reds is None:
                if self.filetype != "uvh5":
                    raise NotImplementedError(
                        "Redundant group iteration without explicitly setting antpos for filetype "
                        + self.filetype
            ...
            baseline_chunks = chunk_baselines_by_redundant_groups(
                reds=reds, max_chunk_size=Nbls
            )
    ...
    def iterate_over_freqs(self, Nchans=1, freqs=None):
        ...
        if freqs is None:
            if self.filetype != "uvh5":
                raise NotImplementedError(
                    "Frequency iteration for filetype "
                    + self.filetype
    ...
    def init_HERACal(self, gain_convention="divide", cal_style="redundant"):
        ...
        uvc = UVCal().initialize_from_uvdata(
            self, gain_convention="divide", cal_style="redundant"
        )
        ...
        uvc.gain_array = np.ones(
            (uvc.Nants_data, uvc.Nfreqs, uvc.Ntimes, uvc.Njones), dtype=np.complex64
        )
        uvc.flag_array = np.ones(
            (uvc.Nants_data, uvc.Nfreqs, uvc.Ntimes, uvc.Njones), dtype=bool
        )
        uvc.quality_array = np.zeros(
            (uvc.Nants_data, uvc.Nfreqs, uvc.Ntimes, uvc.Njones), dtype=np.float32
        )
        uvc.total_quality_array = np.zeros(
            (uvc.Nfreqs, uvc.Ntimes, uvc.Njones), dtype=np.float32
        )
    ...
    def empty_arrays(self):
        ...
        self.data_array = (
            np.zeros_like(self.data_array) if self.data_array is not None else None
        )
        self.flag_array = (
            np.ones_like(self.flag_array) if self.flag_array is not None else None
        )
        self.nsample_array = (
            np.zeros_like(self.nsample_array)
...
def read_hera_hdf5(
    ...
    for filename in filenames:
        ...
        with h5py.File(filename, "r") as f:
            ...
            if len(times) == 0:
                if (
                    len(h["freq_array"].shape) == 2
                ):  # old pyuvdata shapes with spectral windows
                ...
                pol_indices = {
                    uvutils.parse_polstr(
                        POL_NUM2STR_DICT[n], x_orientation=x_orient
                    ): cnt
    ...
    if read_nsamples:
        rv["nsamples"] = {
            bl: np.empty((tot_times, nfreqs), dtype=np.float32) for bl in bls
        }
    # bail here if all we wanted was the info
    if len(rv) == 0:
        return {"info": info}
...
class HERADataFastReader:
    ...
    def _adapt_metadata(self, info_dict, skip_lsts=False):
        ...
        info_dict["bls"] = sorted(
            set(
                [
                    ap + (pol,)
                    for ap in info_dict["antpairs"]
                    for pol in info_dict["pols"]
                ]
            )
        )
        XYZ = XYZ_from_LatLonAlt(
            info_dict["latitude"] * np.pi / 180,
            info_dict["longitude"] * np.pi / 180,
            info_dict["altitude"],
        )
        ...
        info_dict["antpos"] = {
            ant: enu for enu, ant in zip(enu_antpos, info_dict["antpos"])
        }
        info_dict["data_antpos"] = {
            ant: info_dict["antpos"][ant] for ant in info_dict["data_ants"]
        }
        info_dict["times"] = np.unique(info_dict["times"])
        info_dict["times_by_bl"] = {
            ap: info_dict["times"] for ap in info_dict["antpairs"]
        }
        info_dict["times_by_bl"].update(
            {(a2, a1): info_dict["times"] for (a1, a2) in info_dict["antpairs"]}
        )
        if not skip_lsts:
            info_dict["lsts"] = JD2LST(
                info_dict["times"],
                info_dict["latitude"],
                info_dict["longitude"],
                info_dict["altitude"],
            )
            info_dict["lsts_by_bl"] = {
                ap: info_dict["lsts"] for ap in info_dict["antpairs"]
            }

    def _HERAData_error(self, *args, **kwargs):
        raise NotImplementedError(
            "HERADataFastReader does not support this method. Try HERAData instead."
        )
    ...
    def _make_datacontainer(self, rv, key="data"):
        ...
        for meta in HERAData.HERAData_metas:
            if meta in rv["info"] and meta not in [
                "pols",
                "antpairs",
                "bls",
            ]:  # these are functions on datacontainers
...
def load_flags(flagfile, filetype="h5", return_meta=False):
    ...
    if filetype not in ["h5", "npz"]:
        ...
        assert (
            uvf.mode == "flag"
        ), f"The input h5-based UVFlag object must be in flag mode, got {uvf.mode}"
        assert (
            np.issubsctype(uvf.polarization_array.dtype, np.signedinteger)
        ...
        if uvf.type == "baseline":  # one time x freq waterfall per baseline
            ...
            for ip, pol in enumerate(uvf.polarization_array):
                if np.issubdtype(uvf.polarization_array.dtype, np.signedinteger):
                    pol = polnum2str(
                        pol, x_orientation=uvf.x_orientation
                    )  # convert to string if possible
                else:
                    pol = ",".join(
                        [
                            polnum2str(int(p), x_orientation=uvf.x_orientation)
                            for p in pol.split(",")
                        ]
                    )
            ...
            for i, ant in enumerate(uvf.ant_array):
                for ip, jpol in enumerate(uvf.polarization_array):
                    if np.issubdtype(uvf.polarization_array.dtype, np.signedinteger):
                        jpol = jnum2str(
                            jpol, x_orientation=uvf.x_orientation
                        )  # convert to string if possible
                    else:
                        jpol = ",".join(
                            [
                                jnum2str(int(p), x_orientation=uvf.x_orientation)
                                for p in jpol.split(",")
                            ]
                        )
            ...
            uvf.type == "waterfall"
        ):  # one time x freq waterfall (per visibility polarization)
            for ip, jpol in enumerate(uvf.polarization_array):
                if np.issubdtype(uvf.polarization_array.dtype, np.signedinteger):
                    jpol = jnum2str(
                        jpol, x_orientation=uvf.x_orientation
        ...
        assert npz["flag_array"].shape[0] == nAntpairs * len(
            times
        ), "flag_array must have flags for all baselines for all times."
        for p, pol in enumerate(pols):
            flag_array = np.reshape(
                npz["flag_array"][:, 0, :, p], (len(times), nAntpairs, len(freqs))
            )
    ...
    for i, f in enumerate(filepaths):
        if filetype == "miriad":
            ...
            with h5py.File(f, mode="r") as _f:
                ...
                time_array = np.ravel(_f["Header"]["time_array"])
                if "lst_array" in _f["Header"]:
                    lst_array = np.ravel(_f["Header"]["lst_array"])
                else:
                    # need to generate lst_array on the fly
                    lst_array = np.ravel(
                        uvutils.get_lst_for_time(
                            _f["Header"]["time_array"],
                            _f["Header"]["latitude"][()],
                            _f["Header"]["longitude"][()],
                            _f["Header"]["altitude"][()],
                        )
                ...
                baseline_array = uvutils.antnums_to_baseline(
                    np.array(_f["Header"]["ant_1_array"]),
                    np.array(_f["Header"]["ant_2_array"]),
                    Nants_telescope=np.array(_f["Header"]["Nants_telescope"]),
                )
                most_common_bl_num = scipy.stats.mode(baseline_array, keepdims=True)[0][
                    0
                ]
                ...
                if len(time_array) > 1:
                    ...
                    warnings.warn(
                        f"{f} has only one time, so we assume that dtime is the minimum "
                        "integration time. This may be incorrect for LST-binned files."
                    )
                    int_time = np.min(_f["Header"]["integration_time"]) / units.day.to(
                        units.si.s
                    )
...
def partial_time_io(
    hd, times=None, time_range=None, lsts=None, lst_range=None, **kwargs
):
    ...
    assert (
        hd.filetype == "uvh5"
    ), "This function only works for uvh5-based HERAData objects."
    if (
        np.sum(
            [
                times is not None,
        ...
        raise ValueError(
            "Only one of times, time_range, lsts, and lsts_range can be not None."
        )
    ...
    for f in hd.filepaths:
        ...
        try:
            ...
            if "No elements in time range between " in str(
                err
            ) or "No elements in time_array between " in str(err):
                continue  # no matching times, skip this file
            elif "No elements in LST range between " in str(
                err
            ) or "No elements in lst_array between " in str(err):
...
def save_redcal_meta(
    meta_filename, fc_meta, omni_meta, freqs, times, lsts, antpos, history, clobber=True
):
    ...
    with h5py.File(meta_filename, "w") as outfile:
        ...
        fc_grp["polarity_flips"] = np.array(
            [fc_meta["polarity_flips"][ant] for ant in ant_keys]
        )
        ...
        omni_grp["conv_crit"] = np.array(
            [omni_meta["conv_crit"][pols] for pols in pols_keys]
        )
...
def read_redcal_meta(meta_filename):
    ...
    with h5py.File(meta_filename, "r") as infile:
        ...
        antpos = {
            ant: pos
            for ant, pos in zip(
                infile["header"]["antpos"].attrs["antnums"],
        ...
        fc_meta = {}
        ants = [
            (int(num.tobytes().decode("utf8")), pol.tobytes().decode("utf8"))
            for num, pol in infile["fc_meta"]["dlys"].attrs["ants"]
        ]
        fc_meta["dlys"] = {
            ant: dly for ant, dly in zip(ants, infile["fc_meta"]["dlys"][:, :])
        }
        fc_meta["polarity_flips"] = {
            ant: flips
            for ant, flips in zip(ants, infile["fc_meta"]["polarity_flips"][:, :])
        }
        ...
        omni_meta["chisq"] = {
            pols: chisq
            for pols, chisq in zip(pols_keys, infile["omni_meta"]["chisq"][:, :])
        }
        omni_meta["iter"] = {
            pols: itr for pols, itr in zip(pols_keys, infile["omni_meta"]["iter"][:, :])
        }
        omni_meta["conv_crit"] = {
            pols: cc
            for pols, cc in zip(pols_keys, infile["omni_meta"]["conv_crit"][:, :])
        }
...
def to_HERAData(input_data, filetype="miriad", **read_kwargs):
    ...
    if filetype not in ["miriad", "uvfits", "uvh5"]:
        raise NotImplementedError(
            "Data filetype must be 'miriad', 'uvfits', or 'uvh5'."
        )
    if isinstance(input_data, (str, Path)):  # single visibility data path
        ...
        if np.all(
            [isinstance(i, (str, Path)) for i in input_data]
        ):  # List of visibility data paths
            return HERAData(input_data, filetype=filetype, **read_kwargs)
        elif np.all(
            [isinstance(i, (UVData, HERAData)) for i in input_data]
            ...
            raise TypeError(
                "If input is a list, it must be only strings or only UVData/HERAData objects."
            )
    else:
        raise TypeError(
            "Input must be a UVData/HERAData object, a string, or a list of either."
        )
...
def write_vis(
    ...
    **kwargs,
    ...
    polarization_array = np.array(
        [polstr2num(p, x_orientation=x_orientation) for p in pols]
    )
    ...
    if integration_time is None:
        integration_time = (
            np.ones_like(time_array, dtype=np.float64)
            * np.median(np.diff(np.unique(time_array)))
            * 24
            * 3600.0
        )
...
def update_uvdata(
    uvd, data=None, flags=None, nsamples=None, add_to_history="", **kwargs
):
...
def update_vis(
    ...
    **kwargs,
    ...
    # write out results
    _write_HERAData_to_filetype(
        hd, outfilename, filetype_out=filetype_out, clobber=clobber
    )
...
def to_HERACal(input_cal):
    ...
    if isinstance(input_cal, HERACal):  # single HERACal
        ...
        if np.all(
            [isinstance(ic, (str, Path)) for ic in input_cal]
        ):  # List of calfits paths
            return HERACal(input_cal)
        elif np.all(
            [isinstance(ic, (UVCal, HERACal)) for ic in input_cal]
        ):  # List of UVCal/HERACal objects
            hc = reduce(operator.add, input_cal)
            hc.__class__ = HERACal
            return hc
        else:
            raise TypeError(
                "If input is a list, it must be only strings or only UVCal/HERACal objects."
            )
    else:
        raise TypeError(
            "Input must be a UVCal/HERACal object, a string, or a list of either."
        )
...
def load_cal(input_cal, return_meta=False):
    ...
    if return_meta:
        return (
            ...
            quals,
            total_qual,
            np.array([ant[0] for ant in hc.ants]),
            hc.freqs,
            hc.times,
            hc.pols,
        )
    ...
    **kwargs,
    ...
    if antnums2antnames is None:
        antenna_names = np.array(
            ["ant{}".format(ant_num) for ant_num in antenna_numbers]
        )
    else:
        antenna_names = np.array(
            [antnums2antnames[ant_num] for ant_num in antenna_numbers]
        )
    ...
    jones_array = np.array(
        list(set([jstr2num(k[1], x_orientation=x_orientation) for k in gains.keys()]))
    )
    jones_array = jones_array[np.argsort(np.abs(jones_array))]
    pol_array = np.array(
        [jnum2str(j, x_orientation=x_orientation) for j in jones_array]
    )
    ...
    Ntimes = len(time_array)
    if len(time_array) > 1:
        integration_time = np.median(np.diff(time_array)) * 24.0 * 3600.0
    else:
    ...
    if lsts is None:
        ...
        lst_array = utils.JD2LST(
            times,
            latitude=(tel["latitude"] * 180 / np.pi),
            longitude=(tel["longitude"] * 180 / np.pi),
...
def throw_away_flagged_ants(
    infilename,
    outfilename,
    yaml_file=None,
    throw_away_fully_flagged_data_baselines=False,
    clobber=False,
):
    ...
    if yaml_file is not None:
        ...
        qm_utils.apply_yaml_flags(
            uv=hd,
            a_priori_flag_yaml=yaml_file,
    ...
    history_string += (
        f"Also threw out {antpairs_not_to_keep} because data was fully flagged.\n"
    )
...
def throw_away_flagged_ants_parser():
    ...
    ap = argparse.ArgumentParser(
        description="Throw away baselines whose antennas are flagged in a yaml file or which have all integrations/chans flagged."
    )
    ap.add_argument(
        "infilename",
        type=str,
        help="path to visibility data throw out flagged baselines..",
    )
    ap.add_argument(
        "outfilename",
        type=str,
        help="path to new visibility file to write data with thrown out baselines..",
    )
    ap.add_argument(
        "--yaml_file",
        default=None,
        type=str,
        help="yaml file with list of antennas to throw away.",
    )
    ap.add_argument(
...
def uvdata_from_fastuvh5(
    ...
    **kwargs,
    ...
    if not timefirst:
        ...
        uvd.ant_1_array = np.tile(
            np.array([antpair[0] for antpair in antpairs]), len(times)
        )
        uvd.ant_2_array = np.tile(
            np.array([antpair[1] for antpair in antpairs]), len(times)
        )
    else:
        uvd.time_array = np.tile(times, len(antpairs))
        uvd.lst_array = np.tile(lsts, len(antpairs))
        uvd.ant_1_array = np.repeat(
            np.array([antpair[0] for antpair in antpairs]), len(times)
        )
        uvd.ant_2_array = np.repeat(
            np.array([antpair[1] for antpair in antpairs]), len(times)
        )
