...
def simulate_reflections(
    ...
    if add_cable:
        ...
        for ca, cd, cp in zip(camp, cdelay, cphase):
            cg = hs.sigchain.gen_reflection_gains(
                freqs / 1e9,
                cable_ants,
                amp=[ca for a in cable_ants],
                dly=[cd for a in cable_ants],
                phs=[cp for a in cable_ants],
            )
            for k in cg:
                cable_gains[k] *= cg[k]
    ...
    for i, bl in enumerate(np.unique(uvd.baseline_array)):
        ...
        if add_xtalk:
            if antpair[0] != antpair[1]:
                ...
                xt = hs.sigchain.gen_cross_coupling_xtalk(
                    freqs / 1e9, autocorr, amp=xamp, dly=xdelay, phs=xphase
                )
                xt += hs.sigchain.gen_cross_coupling_xtalk(
                    freqs / 1e9, autocorr, amp=xamp, dly=xdelay, phs=xphase, conj=True
                )
...
class Test_ReflectionFitter_Cables(object):
    ...
    def test_model_auto_reflections(self):
        ...
        RF.model_auto_reflections(
            RF.data, (200, 300), keys=[bl_k], window="blackmanharris", ref_sig_cut=100
        )
    ...
    def test_write_auto_reflections(self):
        ...
        uvc = RF.write_auto_reflections(
            "./ex.calfits", input_calfits="./ex.calfits", overwrite=True
        )
        RF._clear_ref()
        RF.model_auto_reflections(
            RF.data,
        ...
        uvc = RF.write_auto_reflections(
            "./ex.calfits",
            time_array=RF.avg_times,
            input_calfits="./ex.calfits",
            overwrite=True,
        )
        assert uvc.Ntimes == 100
...
class Test_ReflectionFitter_XTalk(object):
    ...
    def test_svd_functions(self):
        ...
        assert np.isclose(
            RF.pcomp_model_fft[bl][:, 0], 0
        ).all()  # assert edgecut channels are zeroed
    ...
    def test_misc_svd_funcs(self):
        ...
        assert (
            np.mean(
                np.abs(RF.ufft[bl][select, 0] - RF.uifft[bl][select, 0])
                / np.abs(RF.ufft[bl][select, 0])
            )
            < 0.01
        )
        ...
        for k in umodes:
            umodes[k][:, :10] = RF.umode_interp[k][
                :, :10
            ]  # fill in umodes with smoothed components
