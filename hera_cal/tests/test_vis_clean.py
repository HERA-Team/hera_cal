...
def test_truncate_flagged_edges():
    ...
    xout, dout, wout, edges, _ = vis_clean.truncate_flagged_edges(
        data_in, weights_in, freqs, ax="freq"
    )
    ...
    xout, dout, wout, edges, _ = vis_clean.truncate_flagged_edges(
        data_in, weights_in, times, ax="time"
    )
    ...
    xout, dout, wout, edges, _ = vis_clean.truncate_flagged_edges(
        data_in, weights_in, (times, freqs), ax="both"
    )
...
def test_restore_flagged_edges():
    ...
    freqs = np.arange(64) * 100e3
    # test freq truncation
    xout, dout, wout, edges, chunks = vis_clean.truncate_flagged_edges(
        data_in, weights_in, freqs, ax="freq"
    )
...
def test_find_discontinuity_edges():
    ...
    assert vis_clean.find_discontinuity_edges([0, 1, 2, 4, 5, 6, 7, 9, 11, 12]) == [
        (0, 3),
        (3, 7),
        (7, 8),
        (8, 10),
...
def test_flag_rows_with_flags_within_edge_distance():
    ...
    weights_in[32, 2] = 0.0
    weights_in[33, 12] = 0.0
    weights_in[2, 30] = 0.0
    weights_in[-10, 20] = 0.0
    ...
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        freqs, weights_in, min_flag_edge_distance=3, ax="freq"
    )
    ...
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        freqs, weights_in, min_flag_edge_distance=13, ax="freq"
    )
    for i in range(wout.shape[0]):
        if i == 32 or i == 33:
            assert np.all(np.isclose(wout[i], 0.0))
    ...
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        freqs, weights_in, min_flag_edge_distance=3, ax="time"
    )
    ...
    times = np.arange(Ntimes) * 10.0
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        times, weights_in, min_flag_edge_distance=11, ax="time"
    )
    for i in range(wout.shape[1]):
        if i == 30 or i == 20:
            assert np.all(np.isclose(wout[:, i], 0.0))
    ...
    # now do both
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        [times, freqs], weights_in, min_flag_edge_distance=(3, 3), ax="both"
    )
    for i in range(wout.shape[1]):
        if i == 30:
            assert np.all(np.isclose(wout[:, i], 0.0))
...
def test_flag_rows_with_flags_within_edge_distance_with_breaks():
    ...
    times = np.hstack(
        [
            np.arange(20) * 11.0,
            41 * 11.0 + np.arange(27) * 11.0,
            200 * 11.0 + np.arange(13) * 11.0,
        ]
    )  # time axis with discontinuities at 29 abd 47 integrations
    weights_in = np.outer(np.arange(1, Ntimes + 1), np.arange(1, Nfreqs + 1))
    # frequency direction and time direction separately.
    weights_in[2, 30] = 0.0  # time 2 should not get flagged
    weights_in[21, 48] = 0.0  # time 21 should get flagged
    weights_in[55, 46] = 0.0  # time 55 should get flagged
    weights_in[25, -2] = 0.0  # time 25 should get flagged
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        freqs, weights_in, min_flag_edge_distance=3, ax="freq"
    )
    assert list(np.where(np.all(np.isclose(wout, 0.0), axis=1))[0]) == [21, 25, 55]
    weights_in[22, 30] = 0.0  # channel 30 should be flagged
    # channel 48 will also be flagged.
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        times, weights_in, min_flag_edge_distance=3, ax="time"
    )
    assert list(np.where(np.all(np.isclose(wout, 0.0), axis=0))[0]) == [30, 48]
    weights_in = np.outer(np.arange(1, Ntimes + 1), np.arange(1, Nfreqs + 1))
    # both directions
    weights_in[22, 30] = 0.0  # time 2 should not get flagged
    weights_in[55, 46] = 0.0  # time 55 should get flagged
    weights_in[25, -2] = 0.0  # time 25 should get flagged
    weights_in[22, 30] = 0.0  # channel 30 should be flagged
    wout = vis_clean.flag_rows_with_flags_within_edge_distance(
        [times, freqs], weights_in, min_flag_edge_distance=[2, 3], ax="both"
    )
    assert list(np.where(np.all(np.isclose(wout, 0.0), axis=0))[0]) == [30]
    assert list(np.where(np.all(np.isclose(wout, 0.0), axis=1))[0]) == [25, 55]
...
def test_flag_rows_with_contiguous_flags():
    ...
    wout = vis_clean.flag_rows_with_contiguous_flags(
        weights_in, max_contiguous_flag=(3, 3), ax="both"
    )
    for i in range(wout.shape[1]):
        if i == 30:
            assert np.all(np.isclose(wout[:, i], 0.0))
...
def test_get_max_contiguous_flag_from_filter_periods():
    ...
    filter_half_widths = [[1 / (3.0 * 10)], [1 / (100e3 * 2)]]
    mcf = vis_clean.get_max_contiguous_flag_from_filter_periods(
        freqs, filter_centers[1], filter_half_widths[1]
    )
    assert mcf == 2
    mcf = vis_clean.get_max_contiguous_flag_from_filter_periods(
        times, filter_centers[0], filter_half_widths[0]
    )
    assert mcf == 3
    mcf = vis_clean.get_max_contiguous_flag_from_filter_periods(
        (times, freqs), filter_centers, filter_half_widths
    )
...
class Test_VisClean(object):
    ...
    def test_fourier_filter(self):
        ...
        assert np.all(
            [
                V.clean_info[k][(0, V.Nfreqs)]["status"]["axis_1"][i] == "success"
                for i in V.clean_info[k][(0, V.Nfreqs)]["status"]["axis_1"]
            ]
        )
        ...
        assert pytest.raises(
            ...
            overwrite=True,
            mode="dayenu",
            zeropad=["Mathematical Universe", "Crazy Universe"],
        )
        # check 2d filter.
        V.fourier_filter(
            filter_centers=[fc, fc],
    ...
    def test_vis_clean_dpss(self):
        ...
        for ax in ["freq", "time", "both"]:
            for k in V.flags:
                V.flags[k][:] = False
                V.data[k][:] = np.random.randn(*V.data[k].shape) + 1j * np.random.randn(
                    *V.data[k].shape
                )
    ...
    def test_vis_clean_flag_options(self, tmpdir):
        ...
        V.vis_clean(
            ...
            standoff=0.0,
            min_dly=50.0,
            skip_if_flag_within_edge_distance=(2, 2),
            mode="dpss_leastsq",
        )
        for k in [(24, 25, "ee"), (24, 24, "ee")]:
        ...
        V.vis_clean(
            ...
            standoff=0.0,
            min_dly=50.0,
            skip_if_flag_within_edge_distance=2,
            mode="dpss_leastsq",
        )
        for k in [(24, 25, "ee"), (24, 24, "ee")]:
            for i in range(V.Ntimes):
    ...
    def test_vis_clean_spws(self, tmpdir):
        ...
        V.vis_clean(
            ...
            overwrite=True,
            skip_flagged_edges=True,
            filter_spw_ranges=[(0, 30), (30, 46), (46, V.Nfreqs)],
        )
        for k in V.clean_flags:
            for i in range(V.Nfreqs):
                if i in (0, 28, 29, 30, 31, 32, 33, 46, 47):
    ...
    def test_trim_model(self):
        ...
        for k in V.data:
            V.data[k] = interpolate.interp1d(
                V.freqs, V.data[k], axis=1, fill_value="extrapolate", kind="cubic"
            )(freqs)
        ...
        V.data[k] += noise.sky_noise_jy(
            V.lsts,
            V.freqs / 1e9,
            omega_p=Op,
            integration_time=50,
            autovis=V.data[(23, 23, "ee")],
        )
        ...
        V.vis_clean(
            ...
            keys=[k],
            tol=1e-6,
            min_dly=300.0 * (V.dnu / np.median(np.diff(V.freqs))),
            ax="freq",
            overwrite=True,
            window="tukey",
            alpha=0.2,
        ...
        assert np.all(
            np.isclose(n2[k][-1], n1[k][-1])
        )  # assert non-zeroed output are same as n1 (no polyfit)
    ...
    def test_zeropad(self):
        ...
        d, bval = vis_clean.zeropad_array(
            V.data[(24, 25, "ee")], zeropad=30, axis=0, binvals=V.times
        )
    ...
    def test_time_chunk_from_baseline_chunks(self, tmp_path):
        ...
        for filenum, file in enumerate(datafiles):
            ...
            frf.load_tophat_frfilter_and_write(
                ...
                read_cache=True,
                write_cache=True,
                res_outfilename=fragment_filename,
                clobber=True,
                case="sky",
            )
        ...
        with pytest.warns(RuntimeWarning):
            vis_clean.time_chunk_from_baseline_chunks(
                datafiles[0],
                baseline_chunk_files=datafiles[1:],
                clobber=True,
                outfilename=str(tmp_path / fname),
                time_bounds=True,
            )
