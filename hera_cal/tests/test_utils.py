...
class TestFftDly(object):
    def setup_method(self):
        np.random.seed(0)
        self.freqs = np.linspace(0.1, 0.2, 1024)
    ...
    def test_realistic(self):
        ...
        d, fl, antpos, a, freqs, t, l, p = io.load_vis(
            data_fname, return_meta=True, pick_data_ants=False
        )
        freqs /= 1e9  # in GHz
...
def test_JD2LST():
    ...
    jd = 2458042.0
    assert np.allclose(utils.JD2LST(jd, longitude=21.0), 3.930652307266274)
    # test array execution
    jd = np.arange(2458042, 2458046.1, 0.5)
    lst = utils.JD2LST(jd, longitude=21.0)
    assert len(lst) == 9
...
def test_LST2JD():
    ...
    lsts = np.arange(-np.pi / 4, 7 * np.pi / 2, 0.1)
...
def test_JD2RA():
    ...
    # test array
    jd = np.arange(2458042, 2458043.01, 0.2)
    ra = utils.JD2RA(jd)
    assert len(ra) == 6
...
def test_combine_calfits():
    ...
    utils.combine_calfits(
        [test_file1, test_file2],
        "ex.calfits",
        outdir="./",
        overwrite=True,
        broadcast_flags=True,
    )
    # test it exists
    ...
    assert np.allclose(
        uvc_dly.gain_array[0, 0, 10, 10, 0] * uvc_abs.gain_array[0, 0, 10, 10, 0],
        uvc.gain_array[0, 0, 10, 10, 0],
    )
    if os.path.exists("ex.calfits"):
        os.remove("ex.calfits")
    utils.combine_calfits(
...
def test_lst_rephase_vectorized():
    ...
    data_drift = np.zeros(
        (data.shape[0], len(data.bls()), data.shape[1], len(data._pols)), dtype=complex
    )
...
def test_chisq():
    ...
    data = datacontainer.DataContainer(
        {
            (0, 1, "xx"): np.ones((5, 10), dtype=complex),
    ...
    gains = {
        (0, "Jxx"): 0.5**0.5 * np.ones((5, 10), dtype=complex),
        (1, "Jxx"): 0.5**0.5 * np.ones((5, 10), dtype=complex),
    }
    gain_flags = {
        (0, "Jxx"): np.zeros((5, 10), dtype=bool),
        (1, "Jxx"): np.zeros((5, 10), dtype=bool),
    }
    gain_flags[0, "Jxx"][:, 0] = True
    chisq, nObs, chisq_per_ant, nObs_per_ant = utils.chisq(
        data, model, data_wgts, gains=gains, gain_flags=gain_flags
    )
...
def test_gp_interp1d():
    ...
    yint3 = utils.gp_interp1d(
        times, y, x_eval=times, length_scale=5.0, Nmirror=20, flags=f, nl=1e-10
    )
...
def test_red_average():
    ...
    nsamp = np.sum(
        [hd.get_nsamples(bl + ("ee",)) * ~hd.get_flags(bl + ("ee",)) for bl in reds[0]],
        axis=0,
    )
    ...
    f = np.asarray(
        [
            (~hd.get_flags(bl + ("ee",))).astype(float) * user_weights[(bl + ("ee",))]
            for bl in reds[0]
        ]
    )
    ...
    assert np.isclose(
        data_avg2[blkey], np.mean([data[bl + ("ee",)] for bl in reds[0]], axis=0)
    ).all()
...
def test_gain_relative_difference():
    # setup
    old_gains = {
        (0, "Jxx"): np.ones((10, 10), dtype=complex),
        (1, "Jxx"): np.ones((10, 10), dtype=complex),
    }
    new_gains = {
        (0, "Jxx"): 2.0 * np.ones((10, 10), dtype=complex),
        (1, "Jxx"): 4.0 * np.ones((10, 10), dtype=complex),
    }
    flags = {
        (0, "Jxx"): np.zeros((10, 10), dtype=bool),
    ...
    assert avg_relative_diff["Jxx"][3, 4] == 0.0  # both flagged
    assert avg_relative_diff["Jxx"][3, 5] == 3.0  # ant 0 flagged
    assert avg_relative_diff["Jxx"][4, 3] == 2.0  # ant 1 flagged

    # test different denominator
    relative_diff, avg_relative_diff = utils.gain_relative_difference(
        old_gains, new_gains, flags, denom=new_gains
    )
    assert relative_diff[0, "Jxx"][0, 0] == 0.5
    assert relative_diff[1, "Jxx"][0, 0] == 3.0 / 4.0
    assert avg_relative_diff["Jxx"][0, 0] == 5.0 / 8.0
...
def test_chunck_baselines_by_redundant_group():
    ...
    reds = [
        [(24, 24), (25, 25), (37, 37), (38, 38), (39, 39), (52, 52), (53, 53)],
    ...
    chunked_by_four_expected = [
        reds[0],
        reds[1],
        reds[2],
        reds[3],
        reds[4] + reds[5],
        reds[6] + reds[7] + reds[8],
        reds[9],
    ]
    ...
    for grp in reds:
        for bl in grp:
            bls.append(bl)
    chunked_by_four_output = utils.chunk_baselines_by_redundant_groups(
        reds=filter_reds(reds_extended, bls=bls), max_chunk_size=4
    )
...
def test_select_spw_ranges(tmpdir):
    ...
    utils.select_spw_ranges(
        ...
        spw_ranges=[(0, 256), (332, 364), (792, 1000)],
    )
...
class TestMatchFilesToLSTBins:
    ...
    def test_simple_zero2pi(
        ...
        uvds = self.make_empty_day(
            nfiles=10, time_axis_faster_than_bls=bool(time_axis_faster_than_bls)
        )
    ...
    def test_regex_vs_readtimes(self, tmpdir: Path):
        ...
        lst_edges = (
            uvds[0].lst_array.min(),
            (uvds[5].lst_array.min() + uvds[5].lst_array.max()) / 2,
        )
        out_fls = utils.match_files_to_lst_bins(
            lst_edges=lst_edges,
            file_list=fls,
    ...
    def test_lst_bins_crossing_2pi(self, tmpdir: Path):
        ...
        uvds = self.make_empty_day(
            nfiles=nfiles,
            lst_start=2 * np.pi - total_dlst / 2,
    ...
    def test_one_file_per_lstbin(self, tmpdir):
        ...
        lst_edges = [uvd.lst_array.min() for uvd in uvds]
