...
def test_timeavg_waterfall():
    ...
    # test rephase
    ad, af, an, al, aea = frf.timeavg_waterfall(
        d,
        25,
        flags=f,
        rephase=True,
    ...
    # wrap lst
    ad2, af2, an2, al2, aea2 = frf.timeavg_waterfall(
        d,
        60,
        flags=f,
        rephase=True,
        lsts=lsts + 1.52917804,
        freqs=fr,
        bl_vec=blv,
        nsamples=n,
        extra_arrays=dict(times=t),
    ...
    ad, _, _, _, _ = frf.timeavg_waterfall(
        d, 2, rephase=False, nsamples=n, wgt_by_nsample=False, wgt_by_favg_nsample=True
    )
    np.testing.assert_array_equal(ad[1, :], 1.0)
...
class Test_FRFilter(object):
    ...
    def test_filter_data(self):
        ...
        ec = 0
        np.random.seed(0)
        self.F.data[bl] = np.reshape(
            stats.norm.rvs(0, 1, self.F.Ntimes * self.F.Nfreqs)
    ...
    def test_write_data(self):
        self.F.timeavg_data(
            self.F.data, self.F.times, self.F.lsts, 35, rephase=False, verbose=False
        )
        self.F.write_data(
            self.F.avg_data,
            "./out.uv",
            ...
            times=self.F.avg_times,
            lsts=self.F.avg_lsts,
        )
    ...
    def test_time_avg_data_and_write_baseline_list(self, tmpdir):
        ...
        for file in uvh5s:
            ...
            with pytest.warns(RuntimeWarning):
                frf.time_avg_data_and_write(
                    baseline_list=[],
                    flag_output=output_flags,
                    input_data_list=uvh5s,
                    rephase=True,
                    output_data=output,
                    t_avg=35.0,
                    wgt_by_nsample=True,
    ...
    def test_tophat_frfilter(self, avg_red_bllens, pass_data):
        ...
        frate_centers, frate_half_widths = frf.select_tophat_frates(
            case="sky", uvd=frfil.hd, blvecs=frfil.blvecs
        )
        frfil.tophat_frfilter(
            tol=1e-2,
            output_prefix="frfiltered",
        ...
        for pre_filter in [True, False]:
            ...
            np.testing.assert_array_equal(
                frfil.clean_model[k][:, 0], np.zeros_like(frfil.clean_resid[k][:, 0])
            )
            np.testing.assert_array_equal(
                frfil.clean_resid[k][:, 0], np.zeros_like(frfil.clean_resid[k][:, 0])
            )
    ...
    def test_load_tophat_frfilter_and_write_multifile(self, tmpdir):
        ...
        frf.load_tophat_frfilter_and_write(
            uvh5s, filled_outfilename=outfilename, tol=1e-4, clobber=True, case="sky"
        )
    ...
    def test_get_fringe_rate_limits(self):
        ...
        for bl in c_frs:
            ...
            frates = np.hstack(
                [[myfrf.frates.min() - dfr], myfrf.frates, [myfrf.frates.max() + dfr]]
            )
            frlow, frhigh = (
                frates[np.argmin(np.abs(csum - 0.1))],
                frates[np.argmin(np.abs(csum - 0.9))],
            )
            sim_c_frates[bl] = 0.5 * (frlow + frhigh)
            sim_w_frates[bl] = 0.5 * np.abs(frlow - frhigh)
            sim_w_frates[utils.reverse_bl(bl)] = sim_w_frates[bl]
            sim_c_frates[utils.reverse_bl(bl)] = -sim_c_frates[bl]

        for bl in sim_c_frates:
            assert np.isclose(c_frs[bl], sim_c_frates[bl], atol=0.3, rtol=0.0)
            assert np.isclose(w_frs[bl], sim_w_frates[bl], atol=0.3, rtol=0.0)
        ...
        assert pytest.raises(
            ValueError, frf.get_fringe_rate_limits, uvd, uvb, frate_profiles=profiles
        )
    ...
    def test_load_tophat_frfilter_and_write_beam_frates(self, tmpdir, pre_filter):
        ...
        # perform cleaning.
        frf.load_tophat_frfilter_and_write(
            datafile_list=[test_data],
            beamfitsfile=test_beam,
            mode="dpss_leastsq",
            filled_outfilename=filled_outfilename,
            CLEAN_outfilename=CLEAN_outfilename,
            frate_standoff=0.075,
            pre_filter_modes_between_lobe_minimum_and_zero=pre_filter,
            res_outfilename=resid_outfilename,
    ...
    def test_load_tophat_frfilter_and_write(self, tmpdir):
        ...
        for bl in d:
            assert not np.all(np.isclose(d[bl], 0.0))
    ...
    def test_load_tophat_frfilter_and_write_wgt_by_nsample(self, tmpdir):
        ...
        wgts = datacontainer.DataContainer(
            {k: (~frfil.flags[k]).astype(float) for k in frfil.flags}
        )
        for k in wgts:
    ...
    def test_load_tophat_frfilter_and_write_lst_blacklist(self, tmpdir):
        ...
        frf.load_tophat_frfilter_and_write(
            ...
            avg_red_bllens=False,
            case="sky",
            lst_blacklists=[(23, 6.03), (6.18, 7)],
            blacklist_wgt=1e-3,
        )  # first and last five bins
        hd = io.HERAData(outfilename)
        d, f, n = hd.read(bls=[(53, 54, "ee")])
        for bl in d:
            assert not np.all(np.isclose(d[bl], 0.0))
    ...
    def test_load_tophat_frfilter_and_write_skip_autos(self, tmpdir):
        ...
        for bl in do:
            if bl[0] == bl[1]:
                ...
                assert np.allclose(
                    cd[bl], np.zeros_like(cd[bl])
                )  # check that all model values are zero.
                assert np.allclose(
                    fd[bl][~f[bl]], d[bl][~f[bl]]
                )  # check that filled data equals original data.
            else:
    ...
    def test_load_dayenu_filter_and_write(self, tmpdir):
        ...
        frf.load_tophat_frfilter_and_write(
            uvh5,
            res_outfilename=outfilename,
            max_frate_coeffs=[0.0, 0.025],
            cache_dir=cdir,
            calfile_list=calfile,
            read_cache=True,
            Nbls_per_load=1,
            clobber=True,
            mode="dayenu",
