...
class Test_FRFilter(object):
    ...
    def test_run_fr_filter(self):
        ...
        frfil.run_fr_filter(to_filter=frfil.data.keys(), tol=1e-2)
        for k in frfil.data.keys():
            assert frfil.clean_resid[k].shape == (60, 64)
            assert frfil.clean_model[k].shape == (60, 64)
            assert k in frfil.clean_info
        ...
        for avg_red_bllens in [True, False]:
            ...
            wgts[k][:, 0] = 0.0
            frfil.run_fr_filter(to_filter=[k], weight_dict=wgts, tol=1e-5, window='blackman-harris', skip_wgt=0.1, maxiter=100)
    ...
    def test_load_fr_filter_and_write_baseline_list(self, tmpdir):
        tmp_path = tmpdir.strpath
    ...
    def test_load_fr_filter_and_write(self, tmpdir):
        tmp_path = tmpdir.strpath
        uvh5 = os.path.join(DATA_PATH, "test_input/zen.2458101.46106.xx.HH.OCR_53x_54x_only.uvh5")
        outfilename = os.path.join(tmp_path, 'temp.h5')
        frf.load_fr_filter_and_write(uvh5, res_outfilename=outfilename, tol=1e-4, clobber=True, Nbls_per_load=1)
        hd = io.HERAData(outfilename)
