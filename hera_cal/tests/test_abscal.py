...
class Test_AbsCal_Funcs(object):
    ...
    def test_multiply_gains(self, tmpdir, divide_gains):
        ...
        uvc1 = UVCal()
        uvc1 = uvc1.initialize_from_uvdata(
            self.uvd,
        ...
        uvc2 = UVCal()
        uvc2 = uvc2.initialize_from_uvdata(
            self.uvd,
            gain_convention="divide",
            future_array_shapes=False,
            cal_style="redundant",
        ...
        uvc2.quality_array = np.zeros_like(uvc1.quality_array) + 2.0

        uvc1.write_calfits(gain_1_path, clobber=True)
        uvc2.write_calfits(gain_2_path, clobber=True)

        abscal.multiply_gains(
            gain_1_path,
            gain_2_path,
            output_path,
            clobber=True,
            divide_gains=divide_gains,
        )
    ...
    def test_wiener(self):
        # test smoothing
        d = abscal.wiener(
            self.data, window=(5, 15), noise=None, medfilt=True, medfilt_kernel=(1, 13)
        )
        assert d[(24, 37, "ee")].shape == (60, 64)
        assert d[(24, 37, "ee")].dtype == complex
        # test w/ noise
        d = abscal.wiener(
            self.data, window=(5, 15), noise=0.1, medfilt=True, medfilt_kernel=(1, 13)
        )
        assert d[(24, 37, "ee")].shape == (60, 64)
        # test w/o medfilt
    ...
    def test_avg_data_across_red_bls(self):
        ...
        data, flags, antpos, ants, freqs, times, lsts, pols = io.load_vis(
            self.data_file, return_meta=True
        )
        rd, rf, rk = abscal.avg_data_across_red_bls(
            data, antpos, wgts=wgts, tol=2.0, broadcast_wgts=False
        )
        ...
        wgts[(24, 25, "ee")][45, 45] = 0.0
        rd, rf, rk = abscal.avg_data_across_red_bls(
            data, antpos, tol=2.0, wgts=wgts, broadcast_wgts=True
        )
        ...
        rd, rf, rk = abscal.avg_data_across_red_bls(
            data, antpos, wgts=self.wgts, tol=2.0, mirror_red_data=True
        )
...
class Test_Abscal_Solvers(object):
    ...
    def test_TT_phs_logcal_1pol_assume_2D(self):
        ...
        for bl in data:
            data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [0.01, 0.02, 0]))
        data[0, 1, "ee"][0, 0] = np.nan
        data[0, 1, "ee"][0, 1] = np.inf
        model[0, 1, "ee"][0, 0] = np.nan
        model[0, 1, "ee"][0, 1] = np.inf
        fit = abscal.TT_phs_logcal(model, data, antpos, assume_2D=True)
        np.testing.assert_array_almost_equal(fit["Phi_ew_Jee"], 0.01)
        np.testing.assert_array_almost_equal(fit["Phi_ns_Jee"], 0.02)
    ...
    def test_TT_phs_logcal_4pol_assume_2D(self):
        ...
        np.testing.assert_array_almost_equal(fit["Phi_ew"], 0.01)
        np.testing.assert_array_almost_equal(fit["Phi_ns"], 0.02)
    ...
    def test_TT_phs_logcal_1pol_nDim(self):
        ...
        for bl in data:
            data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [0.01, 0.02, 0.03]))
        data[0, 1, "ee"][0, 0] = np.nan
        data[0, 1, "ee"][0, 1] = np.inf
        model[0, 1, "ee"][0, 0] = np.nan
        model[0, 1, "ee"][0, 1] = np.inf
        fit = abscal.TT_phs_logcal(model, data, antpos, assume_2D=False)
        np.testing.assert_array_almost_equal(fit["Phi_0_Jee"], 0.01)
        np.testing.assert_array_almost_equal(fit["Phi_1_Jee"], 0.02)
        np.testing.assert_array_almost_equal(fit["Phi_2_Jee"], 0.03)
    ...
    def test_delay_slope_lincal_1pol_assume_2D(self):
        ...
        true_gains = {
            ant: np.outer(np.ones(2), np.exp(2.0j * np.pi * true_dlys[ant] * (freqs)))
            for ant in ants
        }
        ...
        gains = abscal.delay_slope_lincal(
            model,
            data,
            antpos,
            df=df,
        ...
        for ant in ants:
            np.testing.assert_array_almost_equal(
                rephased_gains[ant], rephased_true_gains[ant], decimal=3
            )
    ...
    def test_RFI_delay_slope_cal(self):
        ...
        for bl in red_data:
            ...
            for key, slope in true_delay_slopes.items():
                if key[2:4] == bl[2]:
                    red_data[bl] *= np.exp(
                        -2j * np.pi * idealized_blvecs[bl][int(key[-1])] * slope * freqs
                    )

        # Solve for delay slopes
        solved_dly_slopes = abscal.RFI_delay_slope_cal(
        ...
        gains = abscal.RFI_delay_slope_cal(
            reds,
            antpos,
            red_data,
            freqs,
    ...
    def test_ndim_fft_phase_slope_solver_1D_ideal_antpos(self):
        ...
        phase_slopes = abscal.ndim_fft_phase_slope_solver(
            data, bl_vecs, assume_2D=False, zero_pad=3, bl_error_tol=1e-8
        )
        for ps, answer in zip(phase_slopes, [-1.2]):
            assert ps.shape == (2, 3)
            np.testing.assert_array_less(np.abs(ps - answer), 0.1)
    ...
    def test_ndim_fft_phase_slope_solver_2D_ideal_antpos(self):
        ...
        bl_vecs = {bl: (antpos[bl[0]] - antpos[bl[1]]) for bl in data}
        for bl in data:
            data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [-1, 0.1]))

        phase_slopes = abscal.ndim_fft_phase_slope_solver(
            data, bl_vecs, assume_2D=False, zero_pad=3, bl_error_tol=1e-8
        )
        for ps, answer in zip(phase_slopes, [-1, 0.1]):
            assert ps.shape == (2, 3)
            np.testing.assert_array_less(np.abs(ps - answer), 0.2)
    ...
    def test_ndim_fft_phase_slope_solver_3D_ideal_antpos(self):
        ...
        bl_vecs = {bl: (antpos[bl[0]] - antpos[bl[1]]) for bl in data}
        for bl in data:
            data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [-1, -0.1, 2.5]))

        phase_slopes = abscal.ndim_fft_phase_slope_solver(
            data, bl_vecs, assume_2D=False, zero_pad=3, bl_error_tol=1e-8
        )
        for ps, answer in zip(phase_slopes, [-1, -0.1, 2.5]):
            assert ps.shape == (2, 3)
            np.testing.assert_array_less(np.abs(ps - answer), 0.2)
    ...
    def test_ndim_fft_phase_slope_solver_assume_2D_real_antpos(self):
        ...
        bl_vecs = {bl: (antpos[bl[0]] - antpos[bl[1]]) for bl in data}
        for bl in data:
            data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [-0.02, 0.03, 0]))

        phase_slopes = abscal.ndim_fft_phase_slope_solver(
            data, bl_vecs, assume_2D=True, zero_pad=3, bl_error_tol=1
        )
        for ps, answer in zip(phase_slopes, [-0.02, 0.03]):
            assert ps.shape == (2, 3)
            np.testing.assert_array_less(np.abs(ps - answer), 0.003)
    ...
    def test_global_phase_slope_logcal_2D(self):
        ...
        for bl in uncal_data:
            uncal_data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [-0.2, 1]))

        # test results when fit is returned
        fit = abscal.global_phase_slope_logcal(
            model, uncal_data, antpos, solver="ndim_fft", assume_2D=False, verbose=False
        )
        fit2 = abscal.global_phase_slope_logcal(
            model, uncal_data, antpos, solver="ndim_fft", assume_2D=True, verbose=False
        )
        np.testing.assert_array_equal(fit["Phi_0_Jee"], fit2["Phi_ew_Jee"])
        np.testing.assert_array_equal(fit["Phi_1_Jee"], fit2["Phi_ns_Jee"])
        for f, answer in zip(fit.values(), [-0.2, 1]):
            assert f.shape == (2, 1)
        ...
        for solver in ["dft", "ndim_fft"]:
            ...
            for i in range(8):
                if i == 0:
                    gains = abscal.global_phase_slope_logcal(
                        model,
                        data,
                        antpos,
                        solver=solver,
            ...
            np.testing.assert_array_almost_equal(
                np.linalg.norm([data[bl] - model[bl] for bl in data]), 0
            )
    ...
    def test_global_phase_slope_logcal_3D(self):
        ...
        bl_vecs = {bl: (antpos[bl[0]] - antpos[bl[1]]) for bl in data}
        for bl in data:
            data[bl] *= np.exp(1.0j * np.dot(bl_vecs[bl], [-0.8, -0.1, 0.5]))
        ...
        for i in range(10):
            ...
            calibrate_in_place(data, gains)
        np.testing.assert_array_almost_equal(
            np.linalg.norm([data[bl] - model[bl] for bl in data]), 0, 5
        )
    ...
    def test_complex_phase_abscal(self):
        ...
        for bi, bls in enumerate(data_bls):
            ...
            data[bls] = model[bls] * np.exp(
                -1j * np.sum(transformed_b_vecs[bi][None, None, :] * phase_deg, axis=-1)
            )
...
class Test_AbsCal(object):
    ...
    def test_merge_gains(self):
        ...
        self.AC.amp_logcal(verbose=False)
        gains = [
            self.AC.abs_eta_gain,
            self.AC.TT_Phi_gain,
            self.AC.abs_psi_gain,
            self.AC.ant_dly_gain,
            self.AC.ant_eta_gain,
            self.AC.ant_phi_gain,
        ]
        gains[0][(99, "Jee")] = 1.0
        # merge shared keys
        ...
        assert np.allclose(
            np.abs(mgains[k][0, 0]),
            np.abs(self.AC.abs_eta_gain[k] * self.AC.ant_eta_gain[k])[0, 0],
        )
        assert np.allclose(
            np.angle(mgains[k][0, 0]),
            np.angle(
                self.AC.TT_Phi_gain[k]
                * self.AC.abs_psi_gain[k]
                * self.AC.ant_dly_gain[k]
                * self.AC.ant_phi_gain[k]
            )[0, 0],
        )
    ...
    def test_mock_data(self):
        ...
        AC.TT_phs_logcal(verbose=False)
        assert np.allclose(
            np.median(AC.abs_eta_arr[0, :, :, 0][AC.wgts[(24, 25, "ee")].astype(bool)]),
            -0.01,
            atol=1e-3,
        )
        assert np.allclose(
    ...
    def test_run_model_based_calibration(self, tmpdir):
        ...
        uvc_precal = uvc_precal.initialize_from_uvdata(
            ...
            gain_convention="divide",
            cal_style="sky",
            ref_antenna_name="Amadeus",
            sky_catalog="The Library of Congress.",
            metadata_only=False,
            sky_field="The Fields of Athenry",
            cal_type="gain",
        )
        uvc_precal.gain_array[:] = 1.0 + 0j
        ...
        hdm.flag_array[np.isclose(hdm.data_array, 0.0)] = True
        hd.flag_array[np.isclose(hd.data_array, 0.0)] = True
        ...
        abscal.run_model_based_calibration(
            ...
            clobber=True,
            precalibration_gain_file=precal_fname,
        )
    ...
    def test_run_model_based_calibration_flagged_gains(self, tmpdir):
        ...
        gains, gain_flags, _, _ = hc.read()
        for k in gains:
            np.testing.assert_array_almost_equal(gains[k][~gain_flags[k]], 1.0)
    ...
    def test_run_model_based_calibration_redundant(self, tmpdir):
        ...
        hdm.select(
            antenna_nums=np.unique(np.hstack([hd.ant_1_array, hd.ant_2_array])),
        ...
        abscal.run_model_based_calibration(
            data_file=red_data_fname,
            model_file=red_model_fname,
            auto_file=red_data_fname,
            output_filename=cal_fname,
            clobber=True,
            refant=(0, "Jnn"),
            constrain_model_to_data_ants=True,
            max_iter=1,
...
class Test_Post_Redcal_Abscal_Run(object):
    def setup_method(self):
        ...
        self.red_data_file = os.path.join(
            DATA_PATH, "test_input/zen.2458098.45361.HH.uvh5_downselected_redavg"
        )
        self.red_model_files = [
            os.path.join(
                DATA_PATH,
                "test_input/zen.2458042.60288.HH.uvRXLS.uvh5_downselected_redavg",
            ),
            os.path.join(
                DATA_PATH,
                "test_input/zen.2458042.61034.HH.uvRXLS.uvh5_downselected_redavg",
            ),
        ]
    ...
    def test_get_d2m_time_map(self):
        ...
        d2m_time_map = abscal.get_d2m_time_map(
            all_data_times, all_data_lsts, all_model_times, all_model_lsts
        )
        ...
        with pytest.raises(ValueError):
            abscal.get_d2m_time_map(
                all_data_times[1:], all_data_lsts, all_model_times, all_model_lsts
            )
        with pytest.raises(ValueError):
            abscal.get_d2m_time_map(
                all_data_times, all_data_lsts, all_model_times[1:], all_model_lsts
            )
    ...
    def test_match_baselines(self):
        ...
        ) = abscal.match_baselines(hd.bls, hdm.bls, hd.antpos)
        ...
        (
            ...
            model_bl_to_load,
            data_to_model_bl_map,
        ) = abscal.match_baselines(data_bls, model_bls, antpos, model_is_redundant=True)
    ...
    def test_build_data_wgts(self):
        ...
        gain_flags = {
            ant: np.zeros((3, 4), dtype=bool)
            for ant in [(0, "Jee"), (1, "Jee"), (2, "Jee"), (-1, "Jee")]
        }
    ...
    def test_post_redcal_abscal(self):
        ...
        with warnings.catch_warnings():
            ...
            delta_gains = abscal.post_redcal_abscal(
                model, copy.deepcopy(data), wgts, rc_flags_subset, verbose=False
            )
        ...
        with warnings.catch_warnings():
            ...
            delta_gains = abscal.post_redcal_abscal(
                ...
                rc_flags_subset,
                verbose=False,
                use_abs_amp_logcal=False,
                use_abs_amp_lincal=False,
            )
    ...
    def test_post_redcal_abscal_run_units_warning(self, tmpdir):
        ...
        with pytest.warns(RuntimeWarning):
            hca = abscal.post_redcal_abscal_run(
                self.data_file,
                calfile_units,
                [model_units],
                phs_conv_crit=1e-4,
    ...
    def test_post_redcal_abscal_run(self, tmpdir):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            hca_red = abscal.post_redcal_abscal_run(
                self.data_file,
                self.redcal_file,
                self.red_model_files,
                phs_conv_crit=1e-4,
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            hca_red_red = abscal.post_redcal_abscal_run(
                self.red_data_file,
                self.redcal_file,
                self.red_model_files,
                phs_conv_crit=1e-4,
                nInt_to_load=10,
                verbose=False,
                add_to_history="testing3",
                model_is_redundant=True,
                data_is_redsol=True,
                raw_auto_file=self.data_file,
                write_delta_gains=True,
                output_file_delta=output_file_delta,
            )
        ...
        # abscal gains.
        assert np.allclose(
            hcat.gain_array[~hcat.flag_array],
