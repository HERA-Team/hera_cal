...
class TestMethods(object):
    ...
    def test_get_pos_red(self):
        ...
        for ant, r in pos.items():
            pos[ant] += [0, 0, 1 * r[0] - 0.5 * r[1]]
        assert len(om.get_pos_reds(pos)) == 234

        pos = hex_array(7, sep=1, split_core=False, outriggers=0)
        assert len(om.get_pos_reds(pos)) < 234
        assert len(om.get_pos_reds(pos, bl_error_tol=0.1)) == 234
        ...
        assert len(om.get_pos_reds(pos, bl_error_tol=0.99)) > 234
        ...
        # test branch cut
        pos = {
            0: np.array([-0.03, 1.0, 0.0]),
            1: np.array([1.0, 1.0, 0.0]),
            2: np.array([0.03, 0.0, 0.0]),
            3: np.array([1.0, 0.0, 0.0]),
        }
        assert len(om.get_pos_reds(pos, bl_error_tol=0.1)) == 4
    ...
    def test_filter_reds(self):
        ...
        assert red == [
            [(0, 1, "xx"), (2, 3, "xx"), (3, 4, "xx"), (4, 5, "xx"), (5, 6, "xx")],
            [(1, 3, "xx"), (2, 4, "xx"), (3, 5, "xx"), (4, 6, "xx")],
            [(0, 3, "xx"), (1, 4, "xx"), (2, 5, "xx"), (3, 6, "xx")],
            [(0, 4, "xx"), (1, 5, "xx"), (2, 6, "xx")],
            [(0, 5, "xx"), (1, 6, "xx")],
        ]
        # include bls
        ...
        assert red == [
            [(0, 2, "xx"), (1, 3, "xx"), (2, 4, "xx"), (3, 5, "xx"), (4, 6, "xx")],
            [(0, 3, "xx"), (1, 4, "xx"), (2, 5, "xx"), (3, 6, "xx")],
        ]
        # exclude ubls
        red = om.filter_reds(
            reds, ex_ubls=[(0, 2), (1, 4), (4, 5), (0, 5), (2, 3), (0, 6)]
        )
    ...
    def test_filter_reds_max_dim(self):
        ...
        ant_inds = set(
            [ant[0] for red in new_reds for bl in red for ant in split_bl(bl)]
        )
    ...
    def test_add_pol_reds(self):
        ...
        polReds = om.add_pol_reds(
            reds, pols=["xx", "xy", "yx", "yy"], pol_mode="4pol_minV"
        )
    ...
    def test_reds_to_antpos(self):
        ...
        reds = om.get_reds(
            true_antpos, pols=["xx", "yy"], pol_mode="2pol", bl_error_tol=1e-10
        )
        inferred_antpos = om.reds_to_antpos(
            reds,
        )
        for pos in inferred_antpos.values():
            assert len(pos) == 1
        new_reds = om.get_reds(
            inferred_antpos, pols=["xx", "yy"], pol_mode="2pol", bl_error_tol=1e-10
        )
        ...
        reds = om.get_reds(
            true_antpos, pols=["xx"], pol_mode="1pol", bl_error_tol=1e-10
        )
        inferred_antpos = om.reds_to_antpos(reds)
        for pos in inferred_antpos.values():
            assert len(pos) == 2
        new_reds = om.get_reds(
            inferred_antpos, pols=["xx"], pol_mode="1pol", bl_error_tol=1e-10
        )
        for nred in new_reds:
            for red in reds:
                if nred[0] in red:
...
class TestRedSol(object):
    def test_init(self):
        ...
        gains, true_vis, d = sim_red_data(reds, gain_scatter=0.05)
    ...
    def test_setitem(self):
        ...
        rs = om.RedSol(
            reds,
            gains={(0, "Jee"): np.ones((1, 1)), (1, "Jee"): np.ones((1, 1))},
            vis={(0, 1, "ee"): np.ones((1, 1))},
        )
    ...
    def test_update_vis_from_data(self):
        ...
        sol.update_vis_from_data(
            DataContainer(d), wgts={bl: 1.0 + i for i, bl in enumerate(d)}
        )
        for red in reds:
            for bl in red:
                np.testing.assert_array_almost_equal(true_vis[red[0]], sol.vis[bl])
        ...
        sol.update_vis_from_data(
            DataContainer(d), wgts={bl: 0 for i, bl in enumerate(d)}
        )
        for red in reds:
            for bl in red:
                np.testing.assert_array_almost_equal(true_vis[red[0]], sol.vis[bl])
    ...
    def test_extend_gains(self):
        ...
        sol2.gains = {k: v for k, v in sol2.gains.items() if k not in ex_ants}
        sol2.extend_gains(d)
    ...
    def test_remove_degen(self):
        ...
        meta, sol = info.logcal(d)
        sol.remove_degen(
            degen_sol=dict(list(gains.items()) + list(true_vis.items())), inplace=True
        )
        sol2 = sol.remove_degen(
            degen_sol=dict(list(gains.items()) + list(true_vis.items())), inplace=False
        )
        for red in reds:
            for bl in red:
    ...
    def test_len(self):
        ...
        assert len(rs) == 3
        rs = om.RedSol(
            reds, gains={(0, "Jee"): np.ones((1, 1)), (1, "Jee"): np.ones((1, 1))}
        )
        assert len(rs) == 2
    ...
    def test_values(self):
        ...
        rs = om.RedSol(
            reds,
            gains={(0, "Jee"): np.ones((1, 1)), (1, "Jee"): 2 * np.ones((1, 1))},
            vis={(0, 1, "ee"): 3 * np.ones((1, 1))},
        )
    ...
    def test_gain_model_calibrate_bl(self):
        ...
        rs = om.RedSol(
            reds,
            gains={(0, "Jee"): np.ones((1, 1)), (1, "Jee"): 2j * np.ones((1, 1))},
            vis={(0, 1, "ee"): 3 * np.ones((1, 1))},
        )
    ...
    def test_count_redundant_nsamples(self):
        ...
        red_nsamples = om.count_redundant_nsamples(
            nsamples, reds, good_ants=[(0, "Jee"), (1, "Jee"), (3, "Jee")]
        )
        assert len(red_nsamples) == 1
...
class TestRedundantCalibrator(object):
        ...
        for bls in reds:
            ...
            for bl in bls:
                ...
                np.testing.assert_almost_equal(
                    np.angle(d_bl * mdl.conj()), 0, decimal=10
                )
    ...
    def test_omnical(self):
        ...
        reds = om.get_reds(antpos, pols=["xx"], pol_mode="1pol")
        info = om.RedundantCalibrator(reds)
        gains, true_vis, d = sim_red_data(reds, gain_scatter=0.0099999)
        w = dict([(k, 1.0) for k in d.keys()])
        ...
        for wgt_func in [wgt_func1, wgt_func2]:
            ...
            for bls in reds:
                ...
                for bl in bls:
                    ...
                    mdl = sol[(bl[0], "Jxx")] * sol[(bl[1], "Jxx")].conj() * ubl
                    np.testing.assert_almost_equal(
                        np.abs(d_bl), np.abs(mdl), decimal=10
                    )
                    np.testing.assert_almost_equal(
                        np.angle(d_bl * mdl.conj()), 0, decimal=10
                    )
    ...
    def test_omnical64(self):
        ...
        reds = om.get_reds(antpos, pols=["xx"], pol_mode="1pol")
        info = om.RedundantCalibrator(reds)
        gains, true_vis, d = sim_red_data(reds, shape=(2, 1), gain_scatter=0.0099999)
        w = dict([(k, 1.0) for k in d.keys()])
        sol0 = dict([(k, np.ones_like(v)) for k, v in gains.items()])
        sol0.update(info.compute_ubls(d, sol0))
    ...
    def test_omnical_outliers(self):
        ...
        for bls in reds:
            ...
            for bl in bls:
                ...
                np.testing.assert_almost_equal(
                    np.angle(d_bl * mdl.conj()), 0, decimal=2
                )
    ...
    def test_lincal64(self):
        ...
        for bls in reds:
            ...
            for bl in bls:
                ...
                np.testing.assert_almost_equal(np.abs(d_bl), np.abs(mdl), decimal=6)
                np.testing.assert_almost_equal(
                    np.angle(d_bl * mdl.conj()), 0, decimal=6
                )
    ...
    def test_svd_convergence(self):
        for hexnum in (2, 3, 4):
            for dtype in (np.complex64, np.complex128):
                ...
                reds = om.get_reds(antpos, pols=["xx"], pol_mode="1pol")
                rc = om.RedundantCalibrator(reds)
                gains, _, d = sim_red_data(reds, shape=(2, 1), gain_scatter=0.01)
                d = {k: dk.astype(dtype) for k, dk in d.items()}
                w = {k: 1.0 for k in d.keys()}
                ...
                meta, sol = rc.lincal(
                    d, sol0
                )  # should not raise 'np.linalg.linalg.LinAlgError: SVD did not converge'
    ...
    def test_remove_degen_firstcal_1D(self):
        ...
        true_dlys = {
            (i, split_pol(pol)[0]): np.array([[np.dot(xhat, antpos[i]) * dtau_dx]])
            for i in range(len(antpos))
        }
        dlys = om.remove_degen_gains(reds, true_dlys, mode="phase")
        for k in dlys:
            np.testing.assert_almost_equal(dlys[k], 0, decimal=10)
        dlys = om.remove_degen_gains(
            reds, true_dlys, degen_gains=true_dlys, mode="phase"
        )
        for k in dlys:
    ...
    def test_remove_degen_firstcal_2D(self):
        ...
        # put in a linear slope in delays, see that it is taken out
        true_dlys = {
            (i, split_pol(pol)[0]): np.array(
                [
                    [
                        np.dot(xhat, antpos[i]) * dtau_dx
                        + np.dot(yhat, antpos[i]) * dtau_dy
                    ]
                ]
            )
            for i in range(len(antpos))
    ...
    def test_lincal_hex_end_to_end_1pol_with_remove_degen_and_firstcal(self):
        ...
        reds = om.get_reds(antpos, pols=["xx"], pol_mode="1pol")
        rc = om.RedundantCalibrator(reds)
        freqs = np.linspace(0.1, 0.2, 10)
        gains, true_vis, d = sim_red_data(reds, gain_scatter=0.1, shape=(1, len(freqs)))
        fc_delays = {ant: 100 * np.random.randn() for ant in gains.keys()}  # in ns
        fc_gains = {
            ant: np.reshape(np.exp(-2.0j * np.pi * freqs * delay), (1, len(freqs)))
            for ant, delay in fc_delays.items()
        }
        ...
        np.testing.assert_almost_equal(
            meta["chisq"], np.zeros_like(meta["chisq"]), decimal=10
        )
        ...
        meanSqAmplitude = np.mean(
            [
                np.abs(g[key1] * g[key2])
                for key1 in g.keys()
                for key2 in g.keys()
                if key1[1] == "Jxx" and key2[1] == "Jxx" and key1[0] != key2[0]
            ],
            axis=0,
        )
        ...
        np.testing.assert_almost_equal(
            meanSqAmplitude, degenMeanSqAmplitude, decimal=10
        )
    ...
    def test_lincal_hex_end_to_end_4pol_with_remove_degen_and_firstcal(self):
        ...
        rc = om.RedundantCalibrator(reds)
        freqs = np.linspace(0.1, 0.2, 10)
        gains, true_vis, d = sim_red_data(
            reds, gain_scatter=0.09, shape=(1, len(freqs))
        )
        fc_delays = {ant: 100 * np.random.randn() for ant in gains.keys()}  # in ns
        fc_gains = {
            ant: np.reshape(np.exp(-2.0j * np.pi * freqs * delay), (1, len(freqs)))
        ...
        for bls in reds:
            for bl in bls:
                ...
                d_bl = d[bl]
                mdl = (
                    sol[(bl[0], split_pol(bl[2])[0])]
                    * sol[(bl[1], split_pol(bl[2])[1])].conj()
                    * ubl
                )
                np.testing.assert_almost_equal(np.abs(d_bl), np.abs(mdl), decimal=10)
                np.testing.assert_almost_equal(
        ...
        meanSqAmplitude = np.mean(
            [
                ...
                for key1 in g.keys()
                for key2 in g.keys()
                if key1[1] == "Jyy" and key2[1] == "Jyy" and key1[0] != key2[0]
            ],
            axis=0,
        )
        np.testing.assert_almost_equal(meanSqAmplitude, 1, decimal=10)
        ...
        for bls in reds:
            for bl in bls:
                ...
                d_bl = d[bl]
                mdl = (
                    sol_rd[(bl[0], split_pol(bl[2])[0])]
                    * sol_rd[(bl[1], split_pol(bl[2])[1])].conj()
                    * ubl
                )
                np.testing.assert_almost_equal(np.abs(d_bl), np.abs(mdl), decimal=10)
                np.testing.assert_almost_equal(
        ...
        np.testing.assert_almost_equal(
            np.mean(np.angle(gainSols[gainPols == "Jxx"]), axis=0),
            np.mean(np.angle(degenGains[gainPols == "Jxx"]), axis=0),
            decimal=10,
        )
        np.testing.assert_almost_equal(
            np.mean(np.angle(gainSols[gainPols == "Jyy"]), axis=0),
            np.mean(np.angle(degenGains[gainPols == "Jyy"]), axis=0),
            decimal=10,
        )
    ...
    def test_lincal_hex_end_to_end_4pol_minV_with_remove_degen_and_firstcal(self):
        ...
        degenGains = np.array([gains[ant] for ant in ants])
        np.testing.assert_almost_equal(
            np.mean(np.angle(gainSols), axis=0),
            np.mean(np.angle(degenGains), axis=0),
            decimal=10,
        )
        ...
        np.testing.assert_almost_equal(
            np.mean(np.angle(visSols), axis=0),
            np.mean(np.angle(degenVis), axis=0),
            decimal=10,
        )
    ...
    def test_is_redundantly_calibratable(self):
        ...
        pos[0] += [0.5, 0, 0]
        assert not om.is_redundantly_calibratable(pos, bl_error_tol=0.1)
        assert om.is_redundantly_calibratable(pos, bl_error_tol=1)
    ...
    def test_predict_chisq_per_red(self):
        ...
        non_degen_dof_per_ubl = {
            red[0]: len(red) - 1 - nant * (len(red) - 1.0) / (nbl - nubl)
            for red in reds
        }
        for red in reds:
        ...
        np.testing.assert_approx_equal(np.sum(list(chisq_per_red.values())), dof)
        non_degen_dof_per_ubl = {
            red[0]: len(red) - 1 - nant * (len(red) - 1.0) / (nbl / 2 - nubl / 2)
            for red in reds
        }
        for red in reds:
            assert chisq_per_red[red[0]] - non_degen_dof_per_ubl[red[0]] < 1
    ...
    def test_predict_chisq_statistically(self):
        ...
        times = np.linspace(0, 600.0 / 60 / 60 / 24, 60, endpoint=False)
        df = np.median(np.diff(freqs))
        dt = np.median(np.diff(times)) * 3600.0 * 24

        # Simulate redundant data with noise
        noise_var = 0.001
        g, tv, d = sim_red_data(reds, shape=(len(times), len(freqs)), gain_scatter=0.00)
        ants = g.keys()
        n = DataContainer(
            {
                bl: np.sqrt(noise_var / 2)
                * (np.random.randn(*vis.shape) + 1j * np.random.randn(*vis.shape))
                for bl, vis in d.items()
            }
        )
        ...
        for antnum in antpos.keys():
            noisy_data[(antnum, antnum, "xx")] = np.ones(
                (len(times), len(freqs))
            ) * np.sqrt(noise_var * dt * df)
        ...
        cal["gf_firstcal"] = {
            ant: np.zeros_like(g, dtype=bool) for ant, g in cal["fc_gains"].items()
        }
        cal["g_omnical"] = sol.gains
        cal["v_omnical"] = sol.vis
        cal["gf_omnical"] = {
            ant: ~np.isfinite(g) for ant, g in cal["g_omnical"].items()
        }
        cal["vf_omnical"] = DataContainer(
            {bl: ~np.isfinite(v) for bl, v in cal["v_omnical"].items()}
        )
        cal["v_omnical"] = DataContainer(cal["v_omnical"])
        cal["g_omnical"] = {
            ant: g * ~cal["gf_omnical"][ant] + cal["gf_omnical"][ant]
        ...
        for red in reds:
            for bl in red:
                ...
                chisq_per_bl[bl] = (
                    np.abs(d_here - g1 * np.conj(g2) * v_here) ** 2 / noise_var
                )
        ...
        for bl in chisq_per_bl:
            np.testing.assert_almost_equal(
                np.mean(chisq_per_bl[bl]), predicted_chisq_per_bl[bl], -np.log10(0.02)
            )

        predicted_chisq_per_red = om.predict_chisq_per_red(reds)
        for red in chisq_per_red:
            np.testing.assert_almost_equal(
                np.mean(chisq_per_red[red]),
                predicted_chisq_per_red[red],
                -np.log10(0.02),
            )
        ...
        for ant in chisq_per_ant:
            np.testing.assert_almost_equal(
                np.mean(chisq_per_ant[ant]),
                predicted_chisq_per_ant[ant],
                -np.log10(0.02),
            )
    ...
    def test_predict_chisq_statistically_with_excluded_antenna(self):
        ...
        om.expand_omni_gains(
            sol, reds, noisy_data, nsamples, chisq_per_ant=meta["chisq_per_ant"]
        )
        ...
        np.testing.assert_almost_equal(
            np.mean(meta["chisq"]["Jxx"]), 1.0, -np.log10(0.03)
        )

        predicted_chisq_per_bl = om.predict_chisq_per_bl(filtered_reds)
        for bl in predicted_chisq_per_bl:
            np.testing.assert_almost_equal(
                np.mean(chisq_per_bl[bl]), predicted_chisq_per_bl[bl], -np.log10(0.03)
            )
        ...
        for ant in meta["chisq_per_ant"]:
            assert np.mean(meta["chisq_per_ant"][ant]) <= np.mean(
                meta["chisq_per_ant"][6, "Jxx"]
            )
...
class TestRedcalAndAbscal(object):
    def test_post_redcal_abscal(self):
        ...
        gains, true_vis, d = sim_red_data(reds, gain_scatter=0.1, shape=(2, len(freqs)))
        d = DataContainer(d)
        fc_delays = {ant: 100e-9 * np.random.randn() for ant in gains.keys()}  # in s
        fc_offsets = {
            ant: 2 * np.pi * np.random.rand() for ant in gains.keys()
        ...
        for ant in antpos.keys():
            for pol in ["xx"]:
                d[ant, ant, pol] = np.ones_like(
                    d[0, 1, "xx"]
                )  # these are used only for calculating chi^2 and not relevant
        d.freqs = freqs
        d.times_by_bl = {
            bl[0:2]: np.array([2458110.18523274, 2458110.18535701]) for bl in d.keys()
        }
        d.antpos = antpos

        # run redcal
        cal, sol = om.redundantly_calibrate(
            d, reds, oc_conv_crit=1e-13, oc_maxiter=5000, run_logcal=True
        )
        cal["gf_firstcal"] = {
            ant: np.zeros_like(g, dtype=bool) for ant, g in cal["fc_gains"].items()
        }
        ...
        f_omnicaled = DataContainer(
            {bl: np.zeros_like(d[bl], dtype=bool) for bl in d.keys()}
        )
        calibrate_in_place(
            d_omnicaled,
            cal["g_omnical"],
            data_flags=f_omnicaled,
            cal_flags=cal["gf_omnical"],
        )
        wgts = DataContainer(
            {k: (~f_omnicaled[k]).astype(float) for k in f_omnicaled.keys()}
        )
        model = DataContainer({bl: true_vis[red[0]] for red in reds for bl in red})
        model.freqs = freqs
        model.times_by_bl = {
            bl[0:2]: np.array([2458110.18523274, 2458110.18535701])
            for bl in model.keys()
        }
        model.antpos = antpos

        # run abscal
        abscal_delta_gains = abscal.post_redcal_abscal(
            model,
            d_omnicaled,
            wgts,
            cal["gf_omnical"],
        ...
        abscal_gains = {
            ant: cal["g_omnical"][ant] * abscal_delta_gains[ant]
            for ant in cal["g_omnical"]
        }
        refant = {"Jxx": (0, "Jxx"), "Jyy": (0, "Jyy")}
        agr = {
            ant: abscal_gains[ant]
            * np.abs(abscal_gains[refant[ant[1]]])
            / abscal_gains[refant[ant[1]]]
            for ant in abscal_gains.keys()
        }
        tgr = {
            ant: true_gains[ant]
            * np.abs(true_gains[refant[ant[1]]])
            / true_gains[refant[ant[1]]]
...
class TestRunMethods(object):
    def test_get_pol_load_list(self):
        ...
        assert om._get_pol_load_list(["xx", "yy", "xy", "yx"], pol_mode="4pol") == [
            ["xx", "yy", "xy", "yx"]
        ]
        assert om._get_pol_load_list(
            ["xx", "yy", "xy", "yx"], pol_mode="4pol_minV"
        ) == [["xx", "yy", "xy", "yx"]]
    ...
    def test_redundantly_calibrate_with_priors(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            rv1, sol1 = om.redundantly_calibrate(
                data, all_reds, run_logcal=False, run_omnical=False
            )
            rv2, sol2 = om.redundantly_calibrate(
                data, all_reds, sol0=sol1, run_logcal=True, run_omnical=True
            )
            rv3, sol3 = om.redundantly_calibrate(
                data, all_reds, sol0=sol2, run_logcal=False, run_omnical=True
            )
    ...
    def test_expand_omni_sol(self):
        ...
        np.random.seed(21)
        freqs = np.linspace(100e6, 200e6, 64, endpoint=False)
        times = np.linspace(0, 600.0 / 60 / 60 / 24, 3, endpoint=False)
        df = np.median(np.diff(freqs))
        dt = np.median(np.diff(times)) * 3600.0 * 24

        g, tv, d = sim_red_data(reds, shape=(len(times), len(freqs)), gain_scatter=0.01)
        tv, d = DataContainer(tv), DataContainer(d)
        nsamples = DataContainer({bl: np.ones_like(d[bl], dtype=float) for bl in d})

        for antnum in antpos.keys():
            for pol in pols:
                d[(antnum, antnum, pol)] = np.ones(
                    (len(times), len(freqs)), dtype=complex
                )
        d.freqs = deepcopy(freqs)
        d.times_by_bl = {bl[0:2]: deepcopy(times) for bl in d.keys()}

        filtered_reds = om.filter_reds(
            reds, ex_ants=ex_ants, antpos=antpos, max_bl_cut=30
        )
        ...
        om.expand_omni_vis(
            ...
            nsamples,
            chisq=cal["chisq"],
            chisq_per_ant=cal["chisq_per_ant"],
        )
    ...
    def test_redcal_iteration(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_iteration(
                hd, nInt_to_load=1, flag_nchan_high=40, flag_nchan_low=30
            )
        ...
        hd = io.HERAData(
            os.path.join(DATA_PATH, "zen.2458098.43124.downsample.uvh5")
        )  # test w/o partial loading
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_iteration(
                hd, flag_nchan_high=40, flag_nchan_low=30
            )
        _, cal_flags, _, _ = hc_omni.build_calcontainers()
        for t in range(len(hd.times)):
            for flag in cal_flags.values():
        ...
        hd.telescope_location_lat_lon_alt_degrees = (
            -30.7,
            121.4,
            1051.7,
        )  # move array longitude
        redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_iteration(
            hd, solar_horizon=0.0
        )
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_iteration(
                hd, pol_mode="2pol", ex_ants=[1, 27], min_bl_cut=15
            )
        ...
        for pol in ["ee", "nn"]:
            ...
            np.testing.assert_array_equal(
                nsamples[(1, 12, pol)][~flags[(1, 12, pol)]], 4.0
            )
    ...
    def test_redcal_run(self):
        ...
        ant_metrics_file = os.path.join(
            DATA_PATH, "test_input/zen.2458098.43124.HH.uv.ant_metrics.json"
        )
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            ...
            redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_run(
                ...
                a_priori_ex_ants_yaml=os.path.join(
                    DATA_PATH, "test_input", "a_priori_flags_sample.yaml"
                ),
                flag_nchan_low=10,
                flag_nchan_high=10,
                check_after=500,
                iter0_prefix=".iter0",
                metrics_files=ant_metrics_file,
                clobber=True,
            )
            hd = io.HERAData(input_data)
            redcal_meta0, hc_first0, hc_omni0, hd_vissol0 = om.redcal_iteration(
                hd,
                flag_nchan_low=10,
                flag_nchan_high=10,
                check_after=500,
        ...
        for prefix, hc_here, bad_ants in [
            ("", hc_first, [11, 50, 12, 24]),
            (".iter0", hc_first0, [11, 50]),
        ]:
            ...
            (
                ...
                quals_here,
                total_qual_here,
            ) = hc_here.build_calcontainers()
            np.testing.assert_almost_equal(
                np.unique(hc.lst_array), np.unique(hd.lst_array)
            )
            ...
            for antnum, antpos in zip(hc.antenna_numbers, hc.antenna_positions):
                np.testing.assert_almost_equal(
                    antpos, hd.antenna_positions[hd.antenna_numbers == antnum].flatten()
                )
            for ant in gains.keys():
        ...
        for prefix, hc_here, hd_here, bad_ants in [
            ("", hc_omni, hd_vissol, [11, 50, 12, 24]),
            (".iter0", hc_omni0, hd_vissol0, [11, 50]),
        ]:
            ...
            for ant in gains.keys():
                ...
                if not np.all(flags[ant]):
                    np.testing.assert_array_almost_equal(
                        quals[ant][~flags[ant]], quals_here[ant][~flags[ant]]
                    )
                    np.testing.assert_array_almost_equal(
                        gains[ant][~flags[ant]], gains_here[ant][~flags[ant]]
                    )
            ...
            for antpol in total_qual.keys():
                np.testing.assert_array_almost_equal(
                    total_qual[antpol], total_qual_here[antpol]
                )
            assert "testing" in hc.history.replace("\n", "").replace(" ", "")
            if prefix == "":
                # assert 'Throwingoutantenna12' in hc.history.replace('\n', '').replace(' ', '')
            ...
            hd = io.HERAData(
                os.path.splitext(input_data)[0] + prefix + ".omni_vis.uvh5"
            )
            data, flags, nsamples = hd.read()
            ...
            for bl in data_here:
                ...
                if not np.all(flags[bl]):
                    np.testing.assert_array_almost_equal(
                        data[bl][~flags[bl]], data_here[bl][~flags[bl]]
                    )
                    np.testing.assert_array_almost_equal(
                        nsamples[bl][~flags[bl]], nsamples_here[bl][~flags[bl]]
                    )
        ...
        for prefix, meta_here, bad_ants in [
            ("", redcal_meta, [11, 50, 12, 24]),
            (".iter0", redcal_meta0, [11, 50]),
        ]:
            ...
            (
                ...
                antpos,
                history,
            ) = io.read_redcal_meta(meta_file)
            for key1 in fc_meta:
                for key2 in fc_meta[key1]:
                    np.testing.assert_array_almost_equal(
                        fc_meta[key1][key2], meta_here["fc_meta"][key1][key2]
                    )
            for key1 in omni_meta:
                for key2 in omni_meta[key1]:
                    np.testing.assert_array_almost_equal(
                        omni_meta[key1][key2], meta_here["omni_meta"][key1][key2]
                    )
    ...
    def test_redcal_run_bda(self):
        ...
        redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_run(
            uvh5_bda, upsample=True, clobber=True
        )
        ...
        redcal_meta, hc_first, hc_omni, hd_vissol = om.redcal_run(
            uvh5_bda, downsample=True, clobber=True
        )
        gains, _, _, _ = hc_omni.build_calcontainers()
        for gain in gains.values():
        ...
        os.remove(
            os.path.join(
                DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.first.calfits"
            )
        )
        os.remove(
            os.path.join(
                DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.omni.calfits"
            )
        )
        os.remove(
            os.path.join(
                DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.omni_vis.uvh5"
            )
        )
        os.remove(
            os.path.join(
                DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.redcal_meta.hdf5"
            )
        )
