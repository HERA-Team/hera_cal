...
class Test_HERACal(object):
    def setup_method(self):
        ...
        self.fname_t1 = os.path.join(
            DATA_PATH, "test_input/zen.2458101.45361.xx.HH.uv.abs.calfits_54x_only"
        )
        self.fname_t2 = os.path.join(
    ...
    def test_read(self):
        ...
        np.testing.assert_array_equal(
            uvc.gain_array[0, :, :, 0].T,
            gains[9, parse_jpolstr("jxx", x_orientation=hc.x_orientation)],
        )
        np.testing.assert_array_equal(
            uvc.flag_array[0, :, :, 0].T,
            flags[9, parse_jpolstr("jxx", x_orientation=hc.x_orientation)],
        )
        np.testing.assert_array_equal(
            uvc.quality_array[0, :, :, 0].T,
            quals[9, parse_jpolstr("jxx", x_orientation=hc.x_orientation)],
        )
        np.testing.assert_array_equal(
            uvc.total_quality_array[:, :, 0].T,
            total_qual[parse_jpolstr("jxx", x_orientation=hc.x_orientation)],
        )
        ...
        assert hc.pols == [
            parse_jpolstr("jxx", x_orientation=hc.x_orientation),
            parse_jpolstr("jyy", x_orientation=hc.x_orientation),
        ]
...
class Test_HERAData(object):
    def setup_method(self):
        ...
        self.four_pol = [
            os.path.join(DATA_PATH, "zen.2457698.40355.{}.HH.uvcA".format(pol))
    ...
    def test_get_slice(self):
        ...
        np.testing.assert_array_equal(
            hd._get_slice(hd.data_array, (53, 54))[
                parse_polstr("XX", x_orientation=hd.x_orientation)
            ],
            hd.get_data((53, 54, "EE")),
        )
        np.testing.assert_array_equal(
            hd._get_slice(hd.data_array, "EE")[(53, 54)], hd.get_data((53, 54, "EE"))
        )
        ...
        for p in d.pols():
            np.testing.assert_array_almost_equal(
                hd._get_slice(hd.data_array, (80, 81, p)), hd.get_data((80, 81, p))
            )
            np.testing.assert_array_almost_equal(
                hd._get_slice(hd.data_array, (81, 80, p)), hd.get_data((81, 80, p))
            )
    ...
    def test_read_bda_upsample(self):
        ...
        np.testing.assert_array_almost_equal(
            np.diff(d.times_by_bl[117, 118]), 9.66367642 / 24 / 3600
        )
    ...
    def test_read_bda_downsample(self):
        ...
        np.testing.assert_array_almost_equal(
            np.diff(d.times_by_bl[104, 117]), 77.30941133 / 24 / 3600
        )
    ...
    def test_iterate_over_bls(self):
        ...
        for d, f, n in hd.iterate_over_bls(chunk_by_redundant_group=True, Nbls=1):
            ...
            assert np.all(np.isclose(bl_lens - bl_lens[0], 0.0, atol=1.0))
            for dc in (d, f, n):
...
class Test_ReadHeraHdf5(object):
    ...
    def test_basic_read(self):
        for replace in [".uvh5", ".new_shape.uvh5"]:
            rv = io.read_hera_hdf5(
                [
                    self.uvh5_1.replace(".uvh5", replace),
                    self.uvh5_2.replace(".uvh5", replace),
                ],
                read_flags=False,
                read_nsamples=False,
                verbose=True,
    ...
    def test_read_allbls_poltranspose(self):
        for replace in [".uvh5", ".new_shape.uvh5"]:
            rv = io.read_hera_hdf5(
                [self.uvh5_pol.replace(".uvh5", replace)],
                dtype=np.complex128,
                verbose=True,
                check=True,
            )
    ...
    def test_read_bl_then_time_poltranpose(self):
        for replace in [".uvh5", ".new_shape.uvh5"]:
            rv = io.read_hera_hdf5(
                [self.uvh5_blt.replace(".uvh5", replace)],
                verbose=True,
                bls=[(24, 26)],
                check=True,
                read_data=True,
...
class Test_HERADataFastReader:
    ...
    @pytest.mark.parametrize("infile", (["uvh5_1"], ["uvh5_1", "uvh5_2"], "uvh5_h4c"))
    @pytest.mark.parametrize("bls", (None,))
    @pytest.mark.parametrize("pols", (None, ["ee"]))
    ...
    @pytest.mark.parametrize("infile", (["uvh5_1"], "uvh5_h4c"))
...
class Test_Visibility_IO_Legacy(object):
    def test_load_vis(self):
        ...
        assert (
            24,
            24,
            parse_polstr("EE", x_orientation=self.uvd.x_orientation),
        ) in data
    ...
    def test_load_vis_nested(self):
        ...
        for i, j in d:
            for pol in d[i, j]:
                uvpol = list(uvd1.polarization_array).index(
                    polstr2num(pol, x_orientation=uvd1.x_orientation)
                )
                uvmask = np.all(
                    np.array(list(zip(uvd.ant_1_array, uvd.ant_2_array))) == [i, j],
                    axis=1,
                )
                np.testing.assert_equal(
                    d[i, j][pol],
                    np.resize(uvd.data_array[uvmask][:, :, uvpol], d[i, j][pol].shape),
                )
                np.testing.assert_equal(
                    f[i, j][pol],
                    np.resize(uvd.flag_array[uvmask][:, :, uvpol], f[i, j][pol].shape),
                )

        d, f = io.load_vis([filename1, filename2], nested_dict=True)
        for i, j in d:
            for pol in d[i, j]:
                uvpol = list(uvd.polarization_array).index(
                    polstr2num(pol, x_orientation=uvd.x_orientation)
                )
                uvmask = np.all(
                    np.array(list(zip(uvd.ant_1_array, uvd.ant_2_array))) == [i, j],
                    axis=1,
...
class Test_Calibration_IO_Legacy(object):
    def test_load_cal(self):
        ...
        assert sorted(pols) == [
            parse_jpolstr("jxx", x_orientation=cal.x_orientation),
            parse_jpolstr("jyy", x_orientation=cal.x_orientation),
        ]
        ...
        gains, flags, quals, total_qual, ants, freqs, times, pols = io.load_cal(
            [cal_xx, cal_yy], return_meta=True
        )
        assert len(gains.keys()) == 36
        assert len(flags.keys()) == 36
        assert len(quals.keys()) == 36
        ...
        assert times.shape == (3,)
        assert sorted(pols) == [
            parse_jpolstr("jxx", x_orientation=cal_xx.x_orientation),
            parse_jpolstr("jyy", x_orientation=cal_yy.x_orientation),
        ]
    ...
    def test_update_cal(self):
        ...
        # make some modifications
        new_gains = {key: (2.0 + 1.0j) * val for key, val in gains.items()}
        new_flags = {key: np.logical_not(val) for key, val in flags.items()}
        new_quals = {key: 2.0 * val for key, val in quals.items()}
        io.update_cal(
            fname,
            outname,
            gains=new_gains,
...
def test_get_file_times_bda():
    fps = [
        os.path.join(DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.uvh5"),
...
def test_partial_time_io():
    ...
    with pytest.raises(ValueError):
        io.partial_time_io(
            hd,
            lst_range=hd.lsts[hd.filepaths[0]][0:2],
            times=hd.times[hd.filepaths[0]][0:2],
        )
...
def test_baselines_from_filelist_position(tmpdir):
    ...
    filelist_1bl = [
        os.path.join(tmp_path, "zen.2458101.46106.xx.HH.OCR_53x_54x_only.first.uvh5"),
...
class Test_UVDataFromFastUVH5:
    ...
    def test_not_rect(self):
        with pytest.raises(
            NotImplementedError, match="Cannot convert non-rectangular blts to UVData"
        ):
            io.uvdata_from_fastuvh5(self.meta_random)

    def test_lsts_with_start_jd(self):
        uvd = io.uvdata_from_fastuvh5(
            self.meta_default, lsts=np.array([0.1, 0.2]), start_jd=2459887
        )
    ...
    def test_pass_time_and_lst(self):
        with pytest.raises(AssertionError):
            io.uvdata_from_fastuvh5(
                self.meta_default,
                times=np.array([2459887.0, 2459887.1]),
                lsts=np.array([0.1, 0.2, 0.3]),
            )
