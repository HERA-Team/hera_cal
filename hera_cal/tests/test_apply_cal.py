...
class Test_Update_Cal(object):
    ...
    def test_build_gains_by_cadences(self):
        ...
        for Nt in [2, 4, 8]:
            ...
            np.testing.assert_array_equal(
                gains_by_Nt[Nt][(0, "Jnn")],
                np.outer(np.ones(Nt), np.arange(10).astype(complex)),
            )
        ...
        gains_by_Nt, cal_flags_by_Nt = ac.build_gains_by_cadences(
            data, gains, cal_flags=flags, flags_are_wgts=True
        )
        for Nt in [1, 2, 4]:
            assert Nt in gains_by_Nt
            assert Nt in cal_flags_by_Nt
    ...
    def test_calibrate_avg_gains_in_place(self):
        ...
        f_old = {
            (0, "Jxx"): np.random.randn(10, 10) > 0,
            (1, "Jxx"): np.random.randn(10, 10) > 0,
        }
        f_new = {
            (0, "Jxx"): np.random.randn(10, 10) > 0,
            (1, "Jxx"): np.random.randn(10, 10) > 0,
        ...
        # test average
        ac.calibrate_redundant_solution(
            dc,
            flags,
            g_new,
            f_new,
    ...
    def test_calibrate_in_place(self):
        ...
        ac.calibrate_in_place(
            dc, g_new, flags, cal_flags, old_gains=g_old, gain_convention="divide"
        )
        for i in range(10):
            for j in range(10):
                assert np.allclose(
        ...
        flags = DataContainer({(0, 1, "xx"): deepcopy(f)})
        ac.calibrate_in_place(
            dc, g_new, flags, cal_flags, old_gains=g_old, gain_convention="multiply"
        )
        for i in range(10):
            for j in range(10):
                assert np.allclose(
        ...
        with pytest.raises(KeyError):
            ac.calibrate_in_place(
                dc, g_new, flags, cal_flags, old_gains=g_old, gain_convention="blah"
            )
        ...
        del g_new[(0, "Jxx")]
        ac.calibrate_in_place(
            dc, g_new, wgts, cal_flags, gain_convention="divide", flags_are_wgts=True
        )
        ...
        cal_flags_here = {
            ...
            (1, "Jxx"): cal_flags[(1, "Jxx")][0:7, :],
        }
        with pytest.raises(ValueError, match="cal_flags with"):
            ac.calibrate_in_place(
                dc, g_here, data_flags=flags, cal_flags=cal_flags_here, old_gains=None
            )
        old_g_here = {(0, "Jxx"): g0_old[0:8, :], (1, "Jxx"): g1_old[0:8, :]}
        with pytest.raises(ValueError, match="old_gains with"):
            ac.calibrate_in_place(
                dc, g_here, data_flags=flags, cal_flags=None, old_gains=old_g_here
            )
    ...
    def test_apply_cal(self, tmpdir):
        ...
        flags_npz = os.path.join(
            DATA_PATH,
            "test_input/zen.2458101.46106.xx.HH.uvOCR_53x_54x_only.flags.applied.npz",
        )
        ...
        flagged_ints = metrics_io.read_a_priori_int_flags(
    ...
    def test_apply_cal_units(self, tmpdir):
        ...
        hdt.vis_units = "k str"
        uncalibrated_file_homogenous_nsamples_flags = os.path.join(
            tmp_path, "homogenous_nsamples_flags.uvh5"
        )
        hdt.write_uvh5(uncalibrated_file_homogenous_nsamples_flags)
        with pytest.warns(RuntimeWarning):
            ac.apply_cal(
                uncalibrated_file_homogenous_nsamples_flags,
                output,
                calfile,
                clobber=True,
                redundant_average=True,
    ...
    def test_apply_cal_redundant_averaging(self, tmpdir):
        ...
        for bl in wgts:
            if np.all(f[bl]):
                wgts[bl][:] = 0.0
        hda_calibrated = utils.red_average(
            hd_calibrated, reds, inplace=False, wgts=wgts, propagate_flags=True
        )

        ac.apply_cal(
            uncalibrated_file,
            calibrated_redundant_averaged_file,
            calfile,
            gain_convention="divide",
            redundant_average=True,
        )
        ...
        assert np.all(
            np.isclose(
                hda_calibrated.nsample_array,
                hda_calibrated_with_apply_cal.nsample_array,
            )
        )
        assert np.all(
        ...
        with pytest.raises(NotImplementedError):
            ac.apply_cal(
                ...
                calibrated_redundant_averaged_file,
                calfile,
                dont_red_average_flagged_data=True,
                gain_convention="divide",
                redundant_average=True,
                nbl_per_load=2,
                clobber=True,
        ...
        for ngrps in range(3, 6):
            ...
            for m in range(ngrps - 1):
                equal_flags.append(
                    np.all(
                        np.isclose(
                            hda_calibrated_groups[m].flag_array,
                            hda_calibrated_groups[m + 1].flag_array,
                        )
                    )
                )
                equal_times.append(
                    np.all(
                        np.isclose(
                            hda_calibrated_groups[m].time_array,
                            hda_calibrated_groups[m + 1].time_array,
                        )
                    )
                )
                equal_data.append(
                    np.all(
                        np.isclose(
                            hda_calibrated_groups[m].data_array,
                            hda_calibrated_groups[m + 1].data_array,
                        )
                    )
                )
                equal_baselines.append(
                    np.all(
                        np.isclose(
                            hda_calibrated_groups[m].baseline_array,
                            hda_calibrated_groups[m + 1].baseline_array,
                        )
                    )
                )
    ...
    def test_apply_cal_bda(self):
        upsampled_oc = os.path.join(
            DATA_PATH,
            "zen.2459122.30030.sum.bda.downsampled.upsample_in_time.omni.calfits",
        )
        downsampled_oc = os.path.join(
            DATA_PATH,
            "zen.2459122.30030.sum.bda.downsampled.downsample_in_time.omni.calfits",
        )
        ...
        outfile = os.path.join(
            DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.down_calibrated.uvh5"
        )
        ...
        outfile = os.path.join(
            DATA_PATH, "zen.2459122.30030.sum.bda.downsampled.up_calibrated.uvh5"
        )
