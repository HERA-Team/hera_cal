...
def test_get_u_bounds():
    ...
    assert np.isclose(
        u_bounds[0][0], np.min(baseline_lengths) * freqs[0] / nucal.SPEED_OF_LIGHT
    )
    assert np.isclose(
        u_bounds[0][1], np.max(baseline_lengths) * freqs[-1] / nucal.SPEED_OF_LIGHT
    )
...
def test_build_nucal_wgts():
    ...
    abscal_wgts = abscal.build_data_wgts(
        data_flags, data_nsamples, model_flags, autocorrs, auto_flags
    )
    nucal_wgts = nucal.build_nucal_wgts(
        data_flags, data_nsamples, autocorrs, auto_flags, radial_reds, freqs
    )
...
def test_project_u_model_comps_on_spec_axis():
    ...
    freqs = np.linspace(50e6, 250e6, 200)
    spatial_filters = nucal.compute_spatial_filters(radial_reds, freqs)
    spectral_filters = dspec.dpss_operator(freqs, [0], [1e-9], eigenval_cutoff=[1e-9])[
        0
    ].real
    data = {}
    wgts = {}
    ...
    model_comps = nucal.fit_nucal_foreground_model(
        data,
        wgts,
        radial_reds,
        spatial_filters,
    ...
    model2 = nucal.evaluate_foreground_model(
        radial_reds, model_proj, spatial_filters, spectral_filters
    )
...
def test_linear_fit():
    ...
    b1_cached, _ = nucal._linear_fit(
        XTX, Xy, solver="lu_solve", cached_input=cached_input
    )
...
def test_compute_spectral_filters():
    ...
    # Create a design matrix
    spectral_filters = nucal.compute_spectral_filters(
        freqs, spectral_filter_half_width=100e-9, eigenval_cutoff=1e-13
    )
...
def test_evaluate_foreground_model():
    ...
    # Compute the model
    model_comps = nucal.fit_nucal_foreground_model(
        data, data_wgts, radial_reds, spatial_filters, return_model_comps=True
    )
    ...
    with pytest.raises(AssertionError):
        model = nucal.evaluate_foreground_model(
            radial_reds, model_comps, spatial_filters, spectral_filters=spectral_filters
        )
    ...
    with pytest.raises(AssertionError):
        model = nucal.evaluate_foreground_model(
            radial_reds, model_comps, _spatial_filters
        )
...
def test_fit_nucal_foreground_model():
    ...
    freqs = np.linspace(50e6, 250e6, 200)
    spatial_filters = nucal.compute_spatial_filters(radial_reds, freqs)
    spectral_filters = dspec.dpss_operator(freqs, [0], [5e-9], eigenval_cutoff=[1e-9])[
        0
    ].real
    data = {}
    data_wgts = {}
    ...
    u_model = nucal.fit_nucal_foreground_model(
        data, data_wgts, radial_reds, spatial_filters, return_model_comps=False
    )
    ...
    u_model_comps = nucal.fit_nucal_foreground_model(
        data,
        data_wgts,
        radial_reds,
        spatial_filters,
    ...
    for k in model_comps:
        ...
        assert model_comps[k].shape[1:] == (
            spectral_filters.shape[-1],
            spatial_filters[k].shape[-1],
        )
...
class TestGradientDescent:
    def setup_method(self):
        ...
        for ant in ns_antpos:
            if ant > 0:
                self.antpos[ant + 10] = np.array(
                    [ns_antpos[ant][1], ns_antpos[ant][0], 0]
                )
        ...
        self.data = {
            bl: np.sin(
                2
                * np.pi
                * self.radial_reds.baseline_lengths[bl]
                * self.freqs
                / 2.998e8
                * 0.25
            )[None]
            * np.ones((2, 1))
            for rdgrp in self.radial_reds
            for bl in rdgrp
        }
        self.data = DataContainer(self.data)
        self.data_wgts = DataContainer(
            {k: np.ones(self.data[k].shape) for k in self.data}
        )
        ...
        self.init_model_comps = nucal.fit_nucal_foreground_model(
            ...
            self.data_wgts,
            self.radial_reds,
            self.frc.spatial_filters,
            share_fg_model=True,
            return_model_comps=True,
        )
        self.init_model_comps = nucal.project_u_model_comps_on_spec_axis(
            self.init_model_comps, self.frc.spectral_filters
        )

        # Calculate baseline vectors
        self.blvecs = np.array(
            [
                (self.antpos[bl[1]] - self.antpos[bl[0]])[:2]
                for rdgrp in self.radial_reds
                for bl in rdgrp
            ]
        )  # [:, None]
        ...
        self.model_parameters = {
            ...
            "fg_r": [
                self.init_model_comps[rdgrp[0]].real for rdgrp in self.radial_reds
            ],
            "fg_i": [
                self.init_model_comps[rdgrp[0]].imag for rdgrp in self.radial_reds
            ],
        }

        self.spatial_filters = [
            np.array([self.frc.spatial_filters[blkey] for blkey in rdgrp])
            for rdgrp in self.radial_reds
        ]
    ...
    def test_foreground_model(self):
        params = {
            "fg_r": [
                np.random.normal(
                    ...
                    (
                        self.data.shape[0],
                        self.frc.spectral_filters.shape[-1],
                        f.shape[-1],
                    ),
                )
        ...
        model_r, model_i = nucal._foreground_model(
            params, self.frc.spectral_filters, self.spatial_filters
        )
    ...
    def test_mean_squared_error(self):
        ...
        model_r = np.array(
            [model[bl].real for rdgrp in self.radial_reds for bl in rdgrp]
        )
        model_i = np.array(
            [model[bl].imag for rdgrp in self.radial_reds for bl in rdgrp]
        )

        # Separate the real and imaginary components
        data_r = np.array(
            [self.data[bl].real for rdgrp in self.radial_reds for bl in rdgrp]
        )
        data_i = np.array(
            [self.data[bl].imag for rdgrp in self.radial_reds for bl in rdgrp]
        )
        wgts = np.ones_like(data_r)

        # Compute the mean squared error
        mse = nucal._mean_squared_error(
            self.model_parameters, data_r, data_i, wgts, model_r, model_i, self.blvecs
        )
    ...
    def test_calibration_loss_function(self):
        ...
        mse = nucal._calibration_loss_function(
            self.model_parameters,
            data_r,
            data_i,
            wgts,
            self.frc.spectral_filters,
            self.spatial_filters,
            self.blvecs,
        )
    ...
    def test_nucal_post_redcal(self):
        ...
        # Separate the real and imaginary components
        data_r = np.array(
            [amp * self.data[bl].real for rdgrp in self.radial_reds for bl in rdgrp]
        )
        data_i = np.array(
            [amp * self.data[bl].imag for rdgrp in self.radial_reds for bl in rdgrp]
        )
        wgts = np.ones_like(data_r)
        ...
        _, metadata = nucal._nucal_post_redcal(
            ...
            data_i,
            wgts,
            deepcopy(self.model_parameters),
            spectral_filters=self.frc.spectral_filters,
            spatial_filters=self.spatial_filters,
            idealized_blvecs=self.blvecs,
            optimizer=optimizer,
            major_cycle_maxiter=10,
        )
...
class TestSpectrallyRedundantCalibrator:
    ...
    def test_estimate_degeneracies(self):
        ...
        amplitude, tip_tilt = self.frc._estimate_degeneracies(
            data, model, self.data_wgts
        )
        ...
        assert np.isclose(np.sqrt(np.mean(np.square(tip_tilt["nn"]))), 0, atol=1e-5)
    ...
    def test_post_redcal_nucal(self):
        ...
        fit_gains, _model_params, _meta, _ = self.frc.post_redcal_nucal(
            dc,
            self.data_wgts,
            spatial_estimate_only=True,
            minor_cycle_maxiter=3,
            share_fg_model=True,
            major_cycle_maxiter=250,
            spectral_filter_half_width=5e-9,
            estimate_degeneracies=True,
            return_model=True,
        )
