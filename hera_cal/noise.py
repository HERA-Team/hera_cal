...
def predict_noise_variance_from_autos(bl, data, dt=None, df=None, nsamples=None):
    ...
    auto_bl1, auto_bl2 = (
        (bl[0], bl[0], join_pol(ap1, ap1)),
        (bl[1], bl[1], join_pol(ap2, ap2)),
    )
...
def write_per_antenna_noise_std_from_autos(
    infile,
    outfile,
    calfile=None,
...
def noise_std_argparser():
    ...
    a = argparse.ArgumentParser(
        description="Read autocorrelations from a .uvh5 file and write predictions for noise standard \
                                             deviations to disk as a .uvh5 file, optionally calibrating"
    )
    a.add_argument(
        "infile",
        type=str,
        help="path to .uvh5 visibility data file from which to extract autocorrelations",
    )
    a.add_argument(
        "outfile",
        type=str,
        help="path to .uvh5 output data file of noise standard deviations",
    )
    a.add_argument(
        "--calfile",
        type=str,
        default=None,
        nargs="+",
        help="optional path to new calibration calfits file (or files) to apply",
    )
    a.add_argument(
        "--gain_convention",
        type=str,
