...
def _check_polarization_consistency(data, gains):
    ...
    if (len(data) > 0) and (len(gains) > 0):
        ...
        if np.all(data_keys_cardinal) and not np.any(gain_keys_cardinal):
            raise KeyError(
                "All the data keys are cardinal (e.g. 'nn' or 'ee'), but none of the gain keys are."
            )
        elif np.all(gain_keys_cardinal) and not np.any(data_keys_cardinal):
            raise KeyError(
                "All the gain keys are cardinal (e.g. 'Jnn' or 'Jee'), but none of the data keys are."
            )


def calibrate_redundant_solution(
    data,
    data_flags,
    new_gains,
    new_flags,
    ...
    for red in all_reds:
        ...
        gain_ratios = [
            old_gains[i, utils.split_pol(pol)[0]]
            * np.conj(old_gains[j, utils.split_pol(pol)[1]])
            / new_gains[i, utils.split_pol(pol)[0]]
            / np.conj(new_gains[j, utils.split_pol(pol)[1]])
            if not ((i == j) and (utils.split_pol(pol)[0] == utils.split_pol(pol)[1]))
            else np.abs(old_gains[i, utils.split_pol(pol)[0]]) ** 2
            / np.abs(new_gains[i, utils.split_pol(pol)[0]]) ** 2
            for (i, j, pol) in red
        ]
        ...
        for n, bl in enumerate(red):
            ...
            gain_ratios[n][
                new_flags[ant1] | new_flags[ant2] | old_flags[ant1] | old_flags[ant2]
            ] = np.nan
...
def build_gains_by_cadences(data, gains, cal_flags=None, flags_are_wgts=False):
    ...
    for Nt in data_Nts:
        ...
        if not np.isclose(power_of_2, np.round(power_of_2)):
            warnings.warn(
                f"Data with {Nt} integrations is inconsistent with BDA by powers of 2 "
    ...
    while True:
        ...
        if cal_flags_by_Nt is not None:
            cal_flags_by_Nt[max_gain_Nt * 2] = {
                ant: cal_flags_by_Nt[max_gain_Nt][ant].repeat(2, axis=0)
    ...
    while True:
        ...
        for ant, gain in gains_by_Nt[min_gain_Nt].items():
            ...
            if cal_flags_by_Nt is not None:
                ...
                gains_by_Nt[min_gain_Nt // 2][ant] = np.ma.average(
                    [even_gains, odd_gains], axis=0, weights=weights
                ).data
            else:
    ...
    for Nt in data_Nts:
        if Nt not in gains_by_Nt:
            warnings.warn(
                f"Data with {Nt} integrations cannot be calibrated with any of gain cadences: {list(gains_by_Nt.keys())}"
            )

    return gains_by_Nt, cal_flags_by_Nt


def calibrate_in_place(
    data,
    new_gains,
    data_flags=None,
    cal_flags=None,
    ...
    new_gains_by_Nt, cal_flags_by_Nt = build_gains_by_cadences(
        data, new_gains, cal_flags=cal_flags, flags_are_wgts=flags_are_wgts
    )
    ...
    for i, j, pol in data.keys():
        ...
        try:
            ...
            raise ValueError(
                f"new_gains with {list(new_gains.values())[0].shape[0]} integrations are incompatible with data with {Nt} integrations."
            )
        cal_flags_here = None
        if cal_flags_by_Nt is not None:
            try:
                ...
                raise ValueError(
                    f"cal_flags with {list(cal_flags.values())[0].shape[0]} integrations are incompatible with data with {Nt} integrations."
                )
        old_gains_here = None
        if old_gains is not None:
            try:
                ...
                raise ValueError(
                    f"old_gains with {list(old_gains.values())[0].shape[0]} integrations are incompatible with data with {Nt} integrations."
                )
...
def apply_cal(
    ...
    upsample=False,
    downsample=False,
    redundant_solution=False,
    bl_error_tol=1.0,
    add_to_history="",
    clobber=False,
    redundant_average=False,
    redundant_weights=None,
    freq_atol=1.0,
    redundant_groups=1,
    dont_red_average_flagged_data=False,
    spw_range=None,
    exclude_from_redundant_mode="data",
    vis_units=None,
    **kwargs,
):
    ...
    if a_priori_flags_yaml is not None:
        ...
        # flag hc
        hc = apply_yaml_flags(hc, a_priori_flags_yaml, ant_indices_only=True)
    ...
    if old_calibration is not None:
        ...
        for f in old_hc.freqs:
            ...
            if np.any(np.isclose(hc.freqs, f, rtol=0.0, atol=freq_atol)):
    ...
    hd = io.HERAData(
        data_infilename, filetype=filetype_in, upsample=upsample, downsample=downsample
    )
    ...
    if nbl_per_load is not None:
        ...
        if dont_red_average_flagged_data:
            raise NotImplementedError(
                "Completely skipping flagged data in redundantly averaged data not implemented for partial I/O!"
            )
        for attribute, value in kwargs.items():
            hd.__setattr__(attribute, value)
        if redundant_average or redundant_solution:
            all_reds = redcal.get_reds(
                hd.data_antpos,
                pols=hd.pols,
        ...
        for data, data_flags, data_nsamples in hd.iterate_over_bls(
            Nbls=nbl_per_load,
            chunk_by_redundant_group=redundant_average,
            reds=all_reds,
            ...
            if redundant_average:
                ...
                utils.red_average(
                    data=data,
                    flags=data_flags,
                    nsamples=data_nsamples,
                    reds=all_red_antpairs,
                    wgts=redundant_weights,
                    inplace=True,
                ...
                if vis_units is None:
                    if hasattr(hc, "gain_scale") and hc.gain_scale is not None:
                        if (
                            hd.vis_units is not None
                            and hc.gain_scale.lower() != "uncalib"
                            and hd.vis_units.lower() != hc.gain_scale.lower()
                        ):
                            warnings.warn(
                                f"Replacing original data vis_units of {hd.vis_units}"
                                f" with calibration vis_units of {hc.gain_scale}",
                                RuntimeWarning,
                            )
    ...
    # full data loading and writing
        ...
        if redundant_average or redundant_solution:
            all_reds = redcal.get_reds(
                data_antpos, pols=pols, bl_error_tol=bl_error_tol, include_autos=True
            )
        else:
            all_reds = []
        if redundant_average:
        ...
        if not redundant_average:
            if vis_units is None:
                if hasattr(hc, "gain_scale") and hc.gain_scale is not None:
                    if (
                        hd.vis_units is not None
                        and hd.vis_units.lower() != "uncalib"
                        and hd.vis_units.lower() != hc.gain_scale.lower()
                    ):
                        warnings.warn(
                            f"Replacing original data vis_units of {hd.vis_units}"
                            " with calibration vis_units of {hc.gain_scale}",
                            RuntimeWarning,
                        )
                    vis_units = hc.gain_scale
            if vis_units is not None:
            ...
            io.update_uvdata(
                hd, data=data, flags=data_flags, add_to_history=add_to_history, **kwargs
            )
            io._write_HERAData_to_filetype(
                hd, data_outfilename, filetype_out=filetype_out, clobber=clobber
            )
            ...
            for red_chunk in range(redundant_groups):
                    ...
                    if dont_red_average_flagged_data and redundant_groups > 1:
                        grp = [
                            ...
                            if np.any(
                                np.asarray(
                                    [
                                        ~np.isclose(redundant_weights[ap + (pol,)], 0.0)
                                        for pol in data_flags.pols()
                                    ]
                                )
                            )
                        ]
                ...
                data_red, flags_red, nsamples_red = utils.red_average(
                    data=data,
                    flags=data_flags,
                    nsamples=data_nsamples,
                    reds=red_antpairs,
                    red_bl_keys=reds_data_bls,
                    wgts=redundant_weights,
                    inplace=False,
                    propagate_flags=True,
                )
                ...
                if len(reds_data_bls) > 0:
                    ...
                    if filetype_out == "uvh5":
                        if hasattr(hc, "gain_scale") and hc.gain_scale is not None:
                            if (
                                hd_red.vis_units is not None
                                and hd_red.vis_units.lower() != "uncalib"
                                and hd_red.vis_units.lower() != hc.gain_scale.lower()
                            ):
                                warnings.warn(
                                    f"Replacing original data vis_units of {hd.vis_units}"
                                    " with calibration vis_units of {hc.gain_scale}",
...
def apply_cal_argparser():
    ...
    a.add_argument(
        ...
        default=None,
        nargs="+",
        help="path to new calibration calfits file (or files for cross-pol)",
    )
    a.add_argument(
        "--old_cal",
        type=str,
        default=None,
        nargs="+",
        help="path to old calibration calfits file to unapply (or files for cross-pol)",
    )
    a.add_argument(
        "--flag_file",
        type=str,
        default=None,
        help="path to file of flags to OR with data flags",
    )
    a.add_argument(
        "--flag_filetype",
        type=str,
        default="h5",
        help="filetype of flag_file (either 'h5' or legacy 'npz'",
    )
    a.add_argument(
        "--flag_nchan_low",
        type=int,
    ...
    a.add_argument(
        ...
        type=str,
        default=None,
        help="Maximum number of baselines to load at once. uvh5 to uvh5 only."
    ...
    a.add_argument(
        ...
        type=int,
        default=1,
        help="Number of subgroups to split each redundant baseline into for cross power spectra. ",
    )
    a.add_argument(
        "--gain_convention",
        type=str,
    ...
    a.add_argument(
        ...
        default=None,
        type=str,
        help="String to insert into vis_units attribute of output visibility file.",
    )
    a.add_argument(
        "--redundant_average",
        default=False,
        action="store_true",
        help="Redundantly average calibrated data.",
    )
    a.add_argument(
        "--dont_red_average_flagged_data",
        default=False,
        action="store_true",
        help="Do not include flagged data in redundant averages. Prevents redundant groups where one subgroup is flagged.",
    )
    a.add_argument(
        "--spw_range",
        default=None,
        type=int,
        nargs=2,
        help="specify spw range to load.",
    )
    a.add_argument(
        "--exclude_from_redundant_mode",
        default="data",
        type=str,
        help="exclude visibilities from redundant average based on whether entire waterfall is flagged ,'data'"
    ...
    a.add_argument(
        ...
        type=str,
        default=None,
        help="path to yaml file to use in apriori flags.",
    )
    return a
