...
def detect_phase_flips(phases):
    ...
    for i in range(len(complexes)):
        if not np.isnan(complexes[i]):
            flip_here = np.abs(complexes[i] - previous_non_nan) > np.abs(
                complexes[i] + previous_non_nan
            )
...
def solve_2D_DPSS(
    ...
    if ("LU" in cached_input and method == "lu_solve") or (
        "XTXinv" in cached_input and method == "pinv"
    ):
...
def filter_1d(
    ...
    **filter_kwargs,
    ...
    filtered, res, info = hera_filters.dspec.fourier_filter(
        x=xin,
        data=din,
        wgts=win,
        mode=mode,
        filter_centers=[0.0],
        skip_wgt=skip_wgt,
        filter_half_widths=[filter_size],
        filter_dims=fdim,
        cache=cache,
        **filter_kwargs,
    )
...
def time_kernel(nInt, tInt, filter_scale=1800.0):
    ...
    """
    kernel_times = np.append(
        -np.arange(0, nInt * tInt + tInt / 2, tInt)[-1:0:-1],
        np.arange(0, nInt * tInt + tInt / 2, tInt),
    )
    filter_std = filter_scale / (2 * (2 * np.log(2)) ** 0.5)
...
def time_filter(gains, wgts, times, filter_scale=1800.0, nMirrors=0):
    ...
    for n in range(nMirrors):
        ...
        padded_gains = np.vstack(
            (np.flipud(padded_gains[1:, :]), gains, np.flipud(padded_gains[:-1, :]))
        )
        padded_wgts = np.vstack(
            (np.flipud(padded_wgts[1:, :]), wgts, np.flipud(padded_wgts[:-1, :]))
        )
...
def time_freq_2D_filter(
    ...
    **win_kwargs,
    ...
    if method == "DPSS" or method == "dpss_leastsq":
        info = {}
        if skip_flagged_edges:
            xout, gout, wout, edges, chunks = truncate_flagged_edges(
                gains * rephasor, wgts, (times, freqs), ax="both"
            )
        ...
        if filter_mode == "rect":
            area = np.outer(
                np.where(np.abs(fringes) < fringe_scale, 1, 0),
        ...
        CLEAN, info = aipy.deconv.clean(
            image, kernel, tol=tol, area=area, stop_if_div=False, maxiter=maxiter
        )
        filtered = np.fft.fft2(CLEAN + info["res"] * area)
        del info[
            "res"
        ]  # this matches the convention in hera_filters.dspec.high_pass_fourier_filter

    else:
        raise ValueError(
            "Filter method {} not recognized. Must be 'CLEAN' or 'DPSS'".format(method)
        )
...
def pick_reference_antenna(gains, flags, freqs, per_pol=True):
    ...
    for pol in pols if per_pol else [pols]:
        ...
        refant_candidates = sorted(
            [
                ant
                for ant, nflags in flags_per_ant.items()
            ...
            if (pol is None) or (ant[1] == pol):
                if flags is not None:
                    if propagate_refant_flags:
                        ...
                        raise ValueError(
                            "The chosen reference antenna",
                            refant,
                            "is flagged in at least one place where antenna",
                            ant,
                            "is not, so automatic reference antenna selection has failed.",
                        )
                gains[ant] = gains[ant] * refant_phasor


def build_time_blacklist(
    time_grid,
    time_blacklists=[],
    lst_blacklists=[],
    lat_lon_alt_degrees=None,
    telescope_name="HERA",
):
    ...
    if len(lst_blacklists) > 0:
        ...
        if lat_lon_alt_degrees is None:
            if telescope_name.upper() == "HERA":
                lat_lon_alt_degrees = np.array(
                    pyuvdata.utils.LatLonAlt_from_XYZ(utils.HERA_TELESCOPE_LOCATION)
                )
                lat_lon_alt_degrees *= [180 / np.pi, 180 / np.pi, 1]
            else:
                raise NotImplementedError(
                    f"No known position for telescope {telescope_name}. lat_lon_alt_degrees must be specified."
                )
...
class CalibrationSmoother:
    def __init__(
        ...
        blacklist_wgt=0.0,
        pick_refant=False,
        propagate_refant_flags=False,
        freq_threshold=1.0,
        time_threshold=1.0,
        ant_threshold=1.0,
        ignore_calflags=False,
        verbose=False,
    ):
        ...
        if load_cspa:
            self.cspa_grids = {
                ant: np.ones((len(self.time_grid), len(self.freqs)), dtype=float)
                for ant in self.ants
            }
        # Now fill those grid
        for ant in self.ants:
            for cal in self.cals:
                if ant in gains[cal]:
                    ...
                    self.flag_grids[ant][self.time_indices[cal], :] = (
                        False if ignore_calflags else cal_flags[cal][ant]
                    )
                    if load_cspa:
        ...
        flag_threshold_and_broadcast(
            ...
            ant_threshold=time_threshold,
        )

        # build blacklists
        self.blacklist_wgt = blacklist_wgt
        self.time_blacklist = build_time_blacklist(
            self.time_grid,
            time_blacklists=time_blacklists,
            lst_blacklists=lst_blacklists,
            lat_lon_alt_degrees=lat_lon_alt_degrees,
            telescope_name=hc.telescope_name,
        )
        self.freq_blacklist = build_freq_blacklist(
            self.freqs, freq_blacklists=freq_blacklists, chan_blacklists=chan_blacklists
        )
        ...
        if pick_refant:
            ...
            self.refant = pick_reference_antenna(
                self.gain_grids, self.flag_grids, self.freqs, per_pol=True
            )
            utils.echo(
                "\n".join(
                    [
                        "Reference Antenna "
                        + str(self.refant[pol][0])
                        + " selected for "
                        + pol
                        + "."
                        for pol in sorted(list(self.refant.keys()))
                    ]
                ),
    ...
    def check_consistency(self):
        ...
        if len(self.flag_files) > 0:
            all_flag_time_indices = np.array(
                [i for indices in self.flag_time_indices.values() for i in indices]
            )
            unq_flag = np.unique(all_flag_time_indices)
            unq_time = np.unique(all_time_indices)
            assert (
                len(unq_flag) == len(unq_time) and np.all(unq_flag == unq_time)
            ), "The number of unique indices for the flag files does not match the calibration files."
            for ff in self.flag_files:
                assert np.all(
                    np.abs(self.flag_freqs[ff] - self.freqs) < 1e-4
    ...
    def rephase_to_refant(self, warn=True, propagate_refant_flags=False):
        ...
        if hasattr(self, "refant"):
            rephase_to_refant(
                self.gain_grids,
                self.refant,
                flags=self.flag_grids,
                propagate_refant_flags=propagate_refant_flags,
            )
        elif warn:
            warnings.warn(
                "No rephasing done because self.refant has not been set.",
                RuntimeWarning,
            )
    ...
    def time_filter(self, filter_scale=1800.0, mirror_kernel_min_sigmas=5):
        ...
        for ant, gain_grid in self.gain_grids.items():
            ...
            if not np.all(self.flag_grids[ant]):
                ...
                self.gain_grids[ant] = time_filter(
                    gain_grid,
                    wgts_grid,
                    self.time_grid,
    ...
    def filter_1d(
        ...
        **filter_kwargs,
        ...
        for ant, gain_grid in self.gain_grids.items():
            ...
            wgts_grid = _build_wgts_grid(
                self.flag_grids[ant], self.time_blacklist, self.freq_blacklist
            )
            if ax == "freq":
                xaxis = self.freqs
            else:
                xaxis = self.time_grid
            self.gain_grids[ant], info = filter_1d(
                gain_grid,
                wgts_grid,
                xaxis,
                ax=ax,
        ...
        for ant in ordered_ant_keys:
            ...
            if not np.all(self.flag_grids[ant]):
                ...
                if method == "DPSS" or method == "dpss_leastsq":
                    ...
                    # Check to see if the grid size changes after removing the flagged edges
                    if skip_flagged_edges:
                        xout, _, _, edges, chunks = truncate_flagged_edges(
                            gain_grid,
                            wgts_grid,
                            (self.time_grid, self.freqs),
                            ax="both",
                        )
                ...
                filtered, info = time_freq_2D_filter(
                    ...
                    maxiter=maxiter,
                    window=window,
                    dpss_vectors=dpss_vectors,
                    eigenval_cutoff=eigenval_cutoff,
                    method=method,
                    fit_method=fit_method,
                    cached_input=cached_input,
                    skip_flagged_edges=skip_flagged_edges,
                    fix_phase_flips=fix_phase_flips,
                    **win_kwargs,
                )
                flipped = info["phase_flips"] == -1
                if np.any(flipped):
                    print(
                        f"{np.sum(flipped)} phase-flipped integrations detected on antenna {ant} between {self.time_grid[flipped][0]} and {self.time_grid[flipped][-1]}."
                    )
    ...
    def write_smoothed_cal(
        self,
        output_replace=(".flagged_abs.", ".smooth_abs."),
        add_to_history="",
        clobber=False,
        **kwargs,
    ):
...
def smooth_cal_argparser():
    ...
    a = argparse.ArgumentParser(
        description="Smooth calibration solutions in time and frequency using the hera_cal.smooth_cal module."
    )
    a.add_argument(
        "calfits_list",
        type=str,
        nargs="+",
        help="list of paths to chronologically sortable calfits files (usually a full day)",
    )
    a.add_argument(
        "--infile_replace",
        type=str,
        default=".flagged_abs.",
        help="substring of files in calfits_list to replace for output files",
    )
    a.add_argument(
        "--outfile_replace",
        type=str,
        default=".smooth_abs.",
        help="replacement substring for output files",
    )
    a.add_argument(
        "--clobber",
        default=False,
        action="store_true",
        help="overwrites existing file at cal_outfile (default False)",
    )
    a.add_argument(
        "--pick_refant",
        default=False,
        action="store_true",
        help="Automatically picks one reference anteanna per polarization. \
    ...
    a.add_argument(
        ...
        default=None,
        type=str,
        help="only run smooth_cal if the first item in the sorted calfits_list\
    ...
    a.add_argument(
        ...
        default=False,
        action="store_true",
        help="Print status updates while smoothing.",
    )
    ...
    flg_opts.add_argument(
        "--flag_file_list",
        type=str,
        nargs="+",
        default=[],
        help="optional list of paths to chronologically\
    ...
    flg_opts.add_argument(
        ...
        type=str,
        default="h5",
        help="filetype of flag_file_list (either 'h5' or legacy 'npz'",
    )
    flg_opts.add_argument(
        "--antflag_thresh",
        default=0.0,
        type=float,
        help="fraction of flagged pixels across all visibilities (with a common antenna) \
    ...
    flg_opts.add_argument(
        ...
        default=0.35,
        type=float,
        help="Finds the times that flagged for all antennas at a single channel but not \
    ...
    flg_opts.add_argument(
        ...
        default=0.5,
        type=float,
        help="Finds the channels that flagged for all antennas at a single time but not \
    ...
    flg_opts.add_argument(
        ...
        default=0.5,
        type=float,
        help="If, after time and freq thesholding and broadcasting, an antenna is left \
    ...
    bkl_opts = a.add_argument_group(
        title="Blacklisting options used for assigning 0 weight (by default) to times/frequencies so that smooth_cal\n"
        "interpolates/extrapoaltes over them (though they aren't necessarily flagged)."
    )
    bkl_opts.add_argument(
        "--time_blacklists",
        type=_pair,
        default=[],
        nargs="+",
        help="space-separated list of dash-separted pairs of times in Julian Day \
    ...
    bkl_opts.add_argument(
        ...
        default=[],
        nargs="+",
        help="space-separated list of dash-separted pairs of LSTs in hours \
    ...
    bkl_opts.add_argument(
        ...
        default=[],
        nargs="+",
        help="space-separated list of dash-separted pairs of channel numbers \
    ...
    bkl_opts.add_argument(
        ...
        default=[],
        nargs="+",
        help="space-separated list of dash-separted pairs of frequencies in Hz \
    ...
    bkl_opts.add_argument(
        ...
        type=float,
        default=0.0,
        help="Relative weight to assign to blacklisted times/freqs compared to 1.0. Default 0.0 \
    ...
    flt_opts.add_argument(
        "--freq_scale",
        type=float,
        default=10.0,
        help="frequency scale in MHz for the low-pass filter\
    ...
    flt_opts.add_argument(
        ...
        type=float,
        default=1800.0,
        help="time scale in seconds, defined analogously to freq_scale (default 1800 s).",
    )
    flt_opts.add_argument(
        "--tol",
        type=float,
        default=1e-9,
        help="CLEAN algorithm convergence tolerance (default 1e-9)",
    )
    flt_opts.add_argument(
        "--filter_mode",
        type=str,
        default="rect",
        help='Mode for CLEAN algorithm that defines the shape of the area that can have\
    ...
    flt_opts.add_argument(
        ...
        type=str,
        default="tukey",
        help='window function for frequency filtering (default "tukey",\
                          see aipy.dsp.gen_window for options',
    )
    flt_opts.add_argument(
        "--maxiter",
    ...
    flt_opts.add_argument(
        ...
        type=float,
        default=0.3,
        help="alpha parameter to use for Tukey window (ignored if window is not Tukey)",
    )
    flt_opts.add_argument(
        "--method",
        type=str,
        default="CLEAN",
        help='Algorithm used to smooth calibration solutions. Default is "CLEAN". "DPSS" uses \
    ...
    flt_opts.add_argument(
        ...
        type=str,
        default=1e-9,
        help="sinc_matrix eigenvalue cutoff to use for included dpss modes. \
                          Only used when the filtering method is 'DPSS'",
    )
    flt_opts.add_argument(
        "--fit_method",
        type=str,
        default="pinv",
        help="Linear least-squares method used for fitting 2D DPSS modes. \
                          Only used when the filtering method is 'DPSS'",
    )
    flt_opts.add_argument(
        ...
        action="store_true",
        default=False,
        help="if True, use DPSS over integrations with flagged edge channels.\
    ...
    flt_opts.add_argument(
        ...
        default="both",
        type=str,
        help="smooth either in 'freq', 'both' (time and freq) axes or 'none' (no smoothing, just picking a refant and writing output).",
    )
    flt_opts.add_argument(
        "--skip_wgt", default=0.1, type=float, help="skip if this fraction is flagged."
    )
