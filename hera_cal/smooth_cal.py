...
import uvtools
import argparse
from hera_cal.abscal import fft_delay
...
def freq_filter(gains, wgts, freqs, filter_scale=10.0, tol=1e-09, window='none', skip_wgt=0.1, maxiter=100):
    ...
    Befor filtering, removes a per-integration delay using abscal.fft_delay, then puts it back in after filtering.
    ...
    (dlys, phi) = fft_delay(gains, wgts, df=sdf*1e9, medfilt=False) # delays are in seconds
...
class Calibration_Smoother():
    ...
    def check_consistency(self):
        ...
        if self.has_prev_cal or self.has_prev_data:
            ...
            # if prev_gain or prev_data is missing an antenna (e.g. because it was in xants and omnical dropped it), treat it as flagged 
            for (ant,pol) in self.gains.keys():
        ...
        if self.has_next_cal or self.has_next_data:
            ...
            # if next_gain or next_data is missing an antenna (e.g. because it was in xants and omnical dropped it), treat it as flagged 
            for (ant,pol) in self.gains.keys():
