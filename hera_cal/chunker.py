...
def chunk_files(
    ...
    **read_kwargs,
    ...
    if type == "data":
        ...
        chunked_files.select(
            freq_chans=np.arange(spw_range[0], spw_range[1]).astype(int), jones=jones
        )
    # throw away fully flagged baselines.
    if throw_away_flagged_ants:
        from hera_qm.utils import apply_yaml_flags

        chunked_files = apply_yaml_flags(
            chunked_files,
            ant_flag_yaml,
            flag_freqs=False,
            flag_times=False,
...
def chunk_parser():
    ...
    ap.add_argument(
        "--type",
        type=str,
        help="Specify whether you are chunking 'gains' or 'data'",
        default="data",
    )
    ap.add_argument(
        "--polarizations",
        ...
        nargs="+",
        default=None,
        help="optional list of polarizations to select.",
    )
    ap.add_argument(
        "--spw_range",
        type=int,
        nargs=2,
        default=None,
        help="optional 2-tuple of frequency channels to select.",
    )
    ap.add_argument(
        "--clobber",
        default=False,
